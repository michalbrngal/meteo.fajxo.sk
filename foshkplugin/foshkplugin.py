...
prgname = "FOSHKplugin"
prgver = "v0.10"
betaver = " Beta 260224"                         # +betaREPLACEver+
...
ShorthttpTimeOut = 3                             # Timout for time critical requests (e.g. addSignalValues)
...
maxViewsize = 8192                               # max size (bytes) for showing tagfiles
MMMtrigger = "MMM"                               # MMM - Identifier for outputting additional fields
...
processingQueue = False                          # current state of processing queue to prevent parallel queue processing
# v0.10 MIYO support
...
inttime = 0
START_TIME = ""
last_sixtime = 0                                 # last time for clientraw

# v0.10 IoT
iotDict = {}                                     # global dict for IoT data
last_iotDict = {}                                # dict with all last IoT values
UDP_TRIGGER = False                              # trigger one-time full UPD update

# v0.10 sun values
sunDict = {}                                     # some additional values like sunrise, sunset, sunheight, sundir, theomaxsr
last_sunDict = {}                                # dict with all last sun values
...
def floatFallback(string, fallback = "null"):                  # with fallback if fallback is a string
  try: out = float(string) if string != None else None
  except ValueError:
    try: out = float(fallback) if fallback != None else None
    except ValueError: out = fallback
  return out
...
def firstLineIs(filename,what):
  with open(filename, "r") as datei: zeile = datei.readline().rstrip()
  return True if what == zeile else False
...
def tprint(s):
  if HIDDEN_FEATURES: print("---> "+str(s))
...
def pushPrint(text):
  if PO_ENABLE:
    myLink = "<a href=\"http://"+LINK_ADR+":"+LBH_PORT+"/FOSHKplugin/help/\">"+LINK_ADR+"</a> "+RUN_DESC+"for ws@" if LINK_ADR != "" and LBH_PORT != "" else ""
...
def pushSend(url, token, user, message):
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      ...
      if r.status_code in range(200,205): okstr = ""
...
def calcVPDf(tempc, humidity, out = "kPa"):                           # calculate VPD from °F & Humidity - output in inHg
  try:
    tempc = (float(tempc)-32)*5/9
    humidity = float(humidity)
    vpd = round(0.61094 * math.exp((17.625 * tempc) / (tempc + 243.04)) * (1 - humidity/100),2)
    if out == "inHg": vpd = round(vpd/3.386,3) 
  except:
    vpd = "Null"
  return(vpd)
...
def inhgtokpa(f,n):                                      # convert inHg to kPa
  return str(round(float(f)*3.386,n))

def kpatoinhga(f,n):                                     # convert kPa to inHg
  return str(round(float(f)/3.386,n))
...
def kmtomi(f,n):                                        # convert km to miles
  return str(round(float(f)/1.609344,n))
...
def checkBattery(d,FWver,battex_arr,sep=" "):
  ...
  for key,value in d.items():
    if isAmbientWeather and ("batt" in key or "batleak" in key) and int(value) == 0 : outstr += key + sep
    # battery-reporting will be same for all weatherstations again after firmware-update of HP2551 v1.6.7
    #elif "EasyWeather" in FWver and "batt" in key and int(value) == 1 : outstr += key + sep
    else:
      if ("wh65batt" in key or "lowbatt" in key or "wh26batt" in key or "wh25batt" in key) and key not in battex_arr and int(value) == 1 : outstr += key + sep
      elif "batt" in key and len(key) == 5 and key not in battex_arr and int(value) == 1 and not isAmbientWeather: outstr += key + sep
      elif ("wh57batt" in key or "pm25batt" in key or "leakbatt" in key or "co2_batt" in key) and key not in battex_arr and int(value) < 2: outstr += key + sep
      elif ("soilbatt" in key or "wh40batt" in key or "wh68batt" in key or "tf_batt" in key or "leaf_batt" in key or "bgtbatt" in key or "soil_ec_batt" in key) and key not in battex_arr and float(value) <= 1.2: outstr += key + sep
      elif ("wh80batt" in key or "wh90batt" in key or "wh85batt" in key) and key not in battex_arr and float(value) < 2.8: outstr += key + sep
      elif "ws1900batt" in key and key not in battex_arr and float(value) < 2.7: outstr += key + sep
      elif "console_batt" in key and key not in battex_arr and float(value) < 2.66: outstr += key + sep
      elif ("ldsbatt" in key or "wn20batt" in key) and key not in battex_arr and float(value) < 2.41: outstr += key + sep
  if len(outstr) >= sep_len and outstr[-sep_len:] == sep: outstr = outstr[:-sep_len]
  return outstr
...
def ptrendText(d_in, what, lang):
  s = "null"
  arr=[
        ["Falling rapidly","Falling","Steady","Rising","Rising rapidly"],
        ["Schnell fallend","Fallend","Stabil","Steigend","Schnell steigend"],
        ["Snel dalend","Dalend","Gelijkblijvend","Stijgend","Snel stijgend"],
      ]
  if lang == "DE": zeile = 1
  elif lang == "NL": zeile = 2
  else: zeile = 0
  value = getfromDict(d_in,[what],{})
  try: 
    s = arr[zeile][(int(value)+2)]
  except: pass
  return(s)                                                    # ptrendText
...
def getGW1100FWinfo(ipaddr, port, cur_ver):
  ...
  try:
    ...
    mac = mac.replace(":","%3A")                               # URL encode
    model,ver = cur_ver.split("_")                             # strip model & version
    if model == "WS3910A": model = "WS3901A"
  except: pass
  data = "id="+mac + "&model="+model + "&time="+str(int(time.time())) + "&user=1" + "&version="+ver
  signdata = data+"@ecowittnet"
  sign = "&sign="+hashlib.md5(signdata.encode('utf-8')).hexdigest().upper()
...
def checkFWUpgrade():
  ...
  if cur_ver != "":                                            # current version is known now
    ...
    elif "GW1200" in cur_ver: model = "GW1200"
    elif "GW2000" in cur_ver: model = "GW2000"
    elif "GW3000" in cur_ver: model = "GW3000"
    elif "EasyWeatherPro" in cur_ver: model = "EasyWeatherPro"
    elif "EasyWeather" in cur_ver: model = "EasyWeather"
    elif "AMBWeatherPro" in cur_ver: model = "AMBWeatherPro"
    ...
    elif "WS3800" in cur_ver: model = "WS3800"
    elif "WS3900" in cur_ver: model = "WS3900"
    elif "WS3910" in cur_ver: model = "WS3910"
    elif "WN1980" in cur_ver: model = "WN1980"
    elif "WS6210" in cur_ver: model = "WS6210"
    elif "WN1820" in cur_ver: model = "WN1820"
    elif "WN1821" in cur_ver: model = "WN1821"
    else: model = "unknown"
    new_devs = ["GW1100","GW1200","GW2000","GW3000","WS3800","WS3900","WS3910","WN1980","WS6210","EasyWeatherPro","AMBWeatherPro","WN1820","WN1821"]
    ...
    try:                                                       # now get firmware information from server
      ...
      debugPrint("retrieve updinfo from " + UPD_URL + " results in " + str(fw_info.status_code))
      if fw_info.status_code == 200:
        ...
        if rmt_ver == "unknown" and (model in new_devs):       # if not offered this way, try the GW1100-way instead
          fw_info = getGW1100FWinfo(WS_IP, WS_PORT, cur_ver)
          try:
            d = json.loads(fw_info.text)
            rmt_ver = cur_ver.split("_")[0]+"_"+d.get("data").get("name")
            rmt_notes = d.get("data").get("content").split("\r\n")
          except: pass
          if myDebug:
        ...
        try:
          if rmt_ver != "unknown" and verStringToNum(rmt_ver) > verStringToNum(cur_ver):
            ...
            elif model in new_devs: use_app = "the <a href=\"http://"+WS_IP+"/\">WebUI</a> or WSView Plus"
            ...
            for i in range(len(rmt_notes)):
              if rmt_notes[i] != "": logPrint("<WARNING> " + rmt_notes[i].strip())
      ...
      debugPrint("except in outer try: " + str(e))
      pass
  return
...
def theoreticalSun(timestamp):                                 # in: timestamp as local time or UTC?
  utcdate = time.gmtime(timestamp)
  seuil = 0.0                                                  # Schwellwert
  latitude = float(COORD_LAT) if COORD_LAT !="" else None
  longitude = float(COORD_LON) if COORD_LON !="" else None
  if latitude is not None and longitude is not None:
    dayofyear = utcdate.tm_yday
    ...
    #tprint("Sonnenhöhe: "+str(hauteur_soleil)+" Winkel: "+str(angle_horaire)+" Deklination: "+str(declinaison)+" tempsolaire: "+str(tempsolaire)+" minutesjour: "+str(minutesjour))
    if float(hauteur_soleil) > 3:                              # Sonnenhöhe
      try:
        ...
        seuil = 0.0
  #tprint("theoreticalSun time:      " + str(timestamp) + "                seuil:  " + str(round(seuil,2)) + " hauteur: " + str(round(hauteur_soleil,1)))
  return seuil                                                 # theoreticalSun (theoretische Solarstrahlung)

def calcSunduration(sr, interval, currtime):                   ## Werner Krenn
  seuil = theoreticalSun(currtime)
  erg = interval if float(sr) > float(SUN_MIN) and float(sr) > seuil else 0
  percent = round(float(sr)*100/seuil,1) if seuil > 0 else 0
  # fuer Bedeckungsgrad: wieviel Prozent an SR von theor. max SR:
  # jedoch ist seuil bereits mit SUN_COEFF heruntergerechnet
  #percent = round(float(sr)*100/(seuil/float(SUN_COEF)),1) if seuil > 0 else 100
  #tprint("*** calcSunduration erg: " + str(erg) + " currtime: " + str(currtime)  + " sr:     "+ str(sr)              + " theosunsr: " + str(round(seuil,2)) + "   %: " + str(percent) + "%   theor. max: " + str(round(float(seuil)/float(SUN_COEF),2)) + " UV: " + getfromDict(last_d_all,["uv","UV"]) + " spread: " + getfromDict(last_d_all,["spread"]) + " clouds: " + getfromDict(last_d_all,["cloudm"]) + " rr: " + getfromDict(last_d_all,["rainratemm"]) + "   SUN_COEF: " + str(SUN_COEF))
  return erg                                                   # calcSunduration

def solarInfo(latitude, longitude, altitude, local_timestamp = None):
  d = {}
  if local_timestamp == None: local_timestamp  = time.time()
  try:
    latitude = float(latitude) if latitude != "" else None
    longitude = float(longitude) if longitude != "" else None
    altitude = float(altitude) if altitude != "" else 0
    # Lokale Zeitstruktur (übergebener Timestamp ist Lokalzeit)
    lt = time.localtime(local_timestamp)
    ...
    # Automatischer UTC-Offset aus Timestamp (in Stunden)
    actual_offset_hours = (local_timestamp - time.mktime(time.gmtime(local_timestamp))) / 3600.0
    # Standard-Zeitzone (ohne DST): tatsächlicher Offset minus DST-Stunde (falls aktiv)
    dst_hours = 1.0 if lt.tm_isdst == 1 else 0.0
    standard_tz_hours = actual_offset_hours - dst_hours
  
    # --- Sonnenaufgang/-untergang (NOAA-Formel) ---
    lng_hour = longitude / 15.0
  
    def calc_sun_event(is_sunrise):
      t = day_of_year + ((6 - lng_hour) / 24.0) if is_sunrise else day_of_year + ((18 - lng_hour) / 24.0)
      M = (0.9856 * t) - 3.289
      ...
      # -0.833°: Refraktion + Sonnenscheibendurchmesser (NOAA-Definition)
      cosH = (math.cos(math.radians(90.833)) - (sinDec * math.sin(math.radians(latitude)))) / (cosDec * math.cos(math.radians(latitude)))
      if cosH > 1 or cosH < -1:
        return None  # Polar-Tag/Nacht
      H = (360 - math.degrees(math.acos(cosH))) if is_sunrise else math.degrees(math.acos(cosH))
      H /= 15.0
      T = H + RA - (0.06571 * t) - 6.622
      UT = (T - lng_hour) % 24
      # In lokale Zeit umrechnen mit tatsächlichem Offset
      local_event_seconds = (UT + actual_offset_hours) * 3600
      midnight_local = time.mktime((year, month, day, 0, 0, 0, 0, 0, -1))
      return midnight_local + local_event_seconds
  
    sunrise_ts = calc_sun_event(True)
    sunset_ts = calc_sun_event(False)
  
    # --- Sonnenhöhe (geo & sichtbar), Azimut, Strahlung ---
    ...
    # Wahre Sonnenzeit: lokale Zeit (Minuten) + EoT + 4*Longitude - 60*Standard-Zeitzone
    true_solar_time = (hour * 60 + eqtime + 4 * longitude - 60 * standard_tz_hours) % 1440
    ...
    # Geometrische Sonnenhöhe (ohne Refraktion)
    elev_geo = math.degrees(
      math.asin(
        math.sin(lat_rad) * math.sin(decl) +
        math.cos(lat_rad) * math.cos(decl) * math.cos(ha_rad)
      )
    )
    # Sichtbare Sonnenhöhe (mit Refraktions-/Scheiben-Korrektur +0.833°)
    elev_vis = elev_geo + 0.833
    ...
    # Strahlung (zwei Varianten)
    #G_sc = 1367  # W/m² - veralteter Standard
    G_sc = 1361  # W/m² - neuer Standard
    #G_sc = 1451  # W/m² - vergleichbar mit sonnenverlauf.de
  
    pressure_ratio = math.exp(-altitude / 8434.5)
  
    # Geometrische Irradiance
    if elev_geo > 0:
      elev_for_am_geo = max(elev_geo, 0.1)
      am_geo = 1 / (math.sin(math.radians(elev_for_am_geo)) + 0.50572 * ((6.07995 + elev_for_am_geo) ** -1.6364))
      irr_geo = G_sc * (0.7 ** ((am_geo * pressure_ratio) ** 0.678))
    else:
      irr_geo = 0.0
  
    # Sichtbare Irradiance (NOAA-Schwelle)
    if elev_vis > 0:
      elev_for_am_vis = max(elev_vis, 0.1)
      am_vis = 1 / (math.sin(math.radians(elev_for_am_vis)) + 0.50572 * ((6.07995 + elev_for_am_vis) ** -1.6364))
      irr_vis = G_sc * (0.7 ** ((am_vis * pressure_ratio) ** 0.678))
    else:
      irr_vis = 0.0
  
    d.update({"sunrise": str(round(sunrise_ts))})
    d.update({"sunset": str(round(sunset_ts))})
    d.update({"sunheight": str(round(elev_geo,2))})
    d.update({"sunheight_vis": str(round(elev_vis,2))})
    d.update({"sundir": str(round(azimuth,2))})
    d.update({"theomaxsr_geo": str(round(irr_geo,2))})
    d.update({"theomaxsr": str(round(irr_vis,2))})
    d.update({"utc_offset_hours": str(round(actual_offset_hours))})
    d.update({"standard_tz_hours": str(round(standard_tz_hours))})
    #d.update({"dst_active": str(lt.tm_isdst)})

    global last_sunDict
    sun_udp = {}

    # only send changed values via UDP to minimise traffic
    for key,value in d.items():
      if UDP_TRIGGER or (not UDP_ONCHANGE) or (UDP_ONCHANGE and (key not in last_sunDict or value != last_sunDict[key])):
        sun_udp.update({key:value})

    # jetzt UDPstr versenden
    if len(sun_udp) > 0:
      UDPstr = "SID=" + defSID + " " + dictToString(sun_udp," ",True,UDP_IGNORE)
      sendUDP(UDPstr)
    last_sunDict = d.copy()
    ...
    debugPrint("solarInfo except: "+str(e))
    pass
  return d                                                     # solarInfo

def solarInfo2(latitude, longitude, altitude, local_timestamp=None, linke_tl=None, solar_constant=1367.0):
  """
  Liefert momentane Solar- und Sonnenstands-Infos als dict für den lokalen Timestamp.
  - ausschließlich math & time
  - DST/Zeitzone automatisch berücksichtigt
  - Clear-Sky-Dämpfung (ESRA-artig): Linke TL skaliert mit Rayleigh-optischer Dicke

  Rückgabe-Dict (Auswahl):
    theomaxsr           : momentane Globalstrahlung (Horizontal, Clear-Sky) [W/m²]
    theosolar           : momentane TOA-GHI (Top of Atmosphere-Global Horizontal Irradiance ohne Dämpfung) [W/m²]
    dni_toa_wm2         : momentane TOA-DNI (Top of Atmosphere-Direct Normal Irradiance normal zur Sonne) [W/m²]
    sunheight           : Sonnenhöhe [°]
    sundir              : Sonnenazimut [°] (0=N, 90=E, 180=S, 270=W)
    solarza             : cos(θz) - solar zenith angle (SZA)
    solartime           : lokale Sonnenzeit (LST) [h]
    sunrise             : lokaler UNIX-Timestamp für Sonnenaufgang (oder None)
    sunset              : lokaler UNIX-Timestamp für Sonnenuntergang (oder None)
    utc_offset          : aktueller UTC-Offset [h]
    daydeg              : Tages-Deklination [°]
    turbidity           : verwendeter Linke-Turbiditätsfaktor
    solar_constant      : verwendete Solarkonstante [W/m²]
  """

  # ---------- Hilfsfunktionen ----------
  def utc_offset_hoursours(ts):
    lt = time.localtime(ts)
    if lt.tm_isdst > 0 and hasattr(time, 'altzone'):
      return -time.altzone / 3600.0  # z.B. -7200 s -> +2 h (CEST)
    else:
      return -time.timezone / 3600.0 # z.B. -3600 s -> +1 h (CET)

  def zones_meridian_deg(ts):
    return utc_offset_hoursours(ts) * 15.0

  def doy(ts):
    return time.localtime(ts).tm_yday

  def pvpmc_E0(d):
    b = 2.0 * math.pi * (d / 365.0)
    return (1.00011
        + 0.034221 * math.cos(b)
        + 0.00128  * math.sin(b)
        + 0.000719 * math.cos(2.0*b)
        + 0.000077 * math.sin(2.0*b))

  def declination_spencer(d):
    g = 2.0 * math.pi * ((d - 1) / 365.0)
    return (0.006918
        - 0.399912 * math.cos(g)
        + 0.070257 * math.sin(g)
  ...
  def elevation_azimuth(lat_deg, dec_rad, H_deg):
    """Elevation [°], Azimut [°] (0=N, 90=E, 180=S, 270=W)."""
    phi = math.radians(lat_deg)
    H   = math.radians(H_deg)

    # Elevation
    sin_el = math.sin(phi)*math.sin(dec_rad) + math.cos(phi)*math.cos(dec_rad)*math.cos(H)
    sin_el = max(-1.0, min(1.0, sin_el))  # numerische Klammerung
    el_rad = math.asin(sin_el)
    el_deg = math.degrees(el_rad)

    # Azimut (Nord-basiert, CW): aus Süd-basiertem atan2 umgerechnet
    sinH = math.sin(H)
    cosH = math.cos(H)
    tanD = math.tan(dec_rad)
    az_south = math.atan2(sinH, (cosH*math.sin(phi) - tanD*math.cos(phi)))  # Süd=0, CCW
    az_north = az_south + math.pi  # auf Nord-basiert, CW
    if az_north < 0.0:
      az_north += 2.0*math.pi
    elif az_north >= 2.0*math.pi:
      az_north -= 2.0*math.pi
    az_deg = math.degrees(az_north)
    return el_deg, az_deg

  def kasten_young_airmass(zenith_deg):
    cz = math.cos(math.radians(zenith_deg))
    denom = cz + 0.50572 * ((96.07995 - zenith_deg) ** -1.6364)
    return 1.0 / denom

  def pressure_from_alt(alt):
    return 101325.0 * math.exp(-alt / 8434.5)  # P0*exp(-z/H)

  def rayleigh_optical_thickness_factor(m):
    # ESRA-nahe δ_r(m): kleine Zahl, stabilisiert TL-Dämpfung
    if m <= 20.0:
      return 1.0 / (6.62960 + 1.75130*m - 0.12020*m*m + 0.00650*m**3 - 0.00013*m**4)
    else:
      return 1.0 / (10.4 + 0.718*m)

  def sunrise_sunset_ts(lat_deg, dec_rad, lon_deg, ts, eot_min):
    """Sonnenaufgang/-untergang als lokale Timestamps (oder None, None)."""
    phi = math.radians(lat_deg)
    h0  = math.radians(-0.833)  # Refraktion + Sonnenradius
    num = math.sin(h0) - math.sin(phi)*math.sin(dec_rad)
    den = math.cos(phi)*math.cos(dec_rad)
    if abs(den) < 1e-12:
      return None, None  # numerischer Schutz
    cosHo = num / den
    # ▶ Robustheit: einklammern in [-1,1] (statt math domain error)
    cosHo = max(-1.0, min(1.0, cosHo))

    Ho_deg = math.degrees(math.acos(cosHo))
    lst_sr = 12.0 - Ho_deg/15.0
    lst_ss = 12.0 + Ho_deg/15.0

    tz_mer  = zones_meridian_deg(ts)
    lon_corr_min = 4.0 * (lon_deg - tz_mer)
    clock_sr = lst_sr - (eot_min + lon_corr_min) / 60.0
    clock_ss = lst_ss - (eot_min + lon_corr_min) / 60.0

    lt = time.localtime(ts); y, m, d = lt.tm_year, lt.tm_mon, lt.tm_mday

    def to_ts(h_decimal):
      h_decimal = max(0.0, min(23.9999, h_decimal))
      hh = int(h_decimal)
      mm = int((h_decimal - hh) * 60.0)
      ss = int(round((((h_decimal - hh) * 60.0) - mm) * 60.0))
      if ss == 60:
        ss = 0; mm += 1
      if mm == 60:
        mm = 0; hh += 1
      return time.mktime((y, m, d, hh, mm, ss, -1, -1, -1))

    return to_ts(clock_sr), to_ts(clock_ss)
  ...
  try:
    ...
    d     = doy(local_timestamp)
    delta   = declination_spencer(d)
    eot_min = equation_of_time_min(d)
  
    lt    = time.localtime(local_timestamp)
    h_clock = lt.tm_hour + lt.tm_min/60.0 + lt.tm_sec/3600.0
  
    tz_mer  = zones_meridian_deg(local_timestamp)
    lon_corr_min = 4.0 * (longitude - tz_mer)
    lst_h   = h_clock + (eot_min + lon_corr_min) / 60.0
    H_deg   = 15.0 * (lst_h - 12.0)
  
    # Sonnenstand jetzt
    el_deg, az_deg = elevation_azimuth(latitude, delta, H_deg)
    cz = math.cos(math.radians(90.0 - el_deg))
  
    # Sunrise/Sunset (lokale Timestamps)
    sr_ts, ss_ts = sunrise_sunset_ts(latitude, delta, longitude, local_timestamp, eot_min)
  
    # Turbidität klären - wenn nicht vorgegeben, dynamisch berechnen
    if linke_tl == None:
      if (latitude >= 0 and (d <= 59 or d >= 334)) or (latitude < 0 and (151 <= d <= 243)):    # Winter
        base_turbidity = 2.5                                                                   # was: 3.0
      elif (latitude >= 0 and (151 <= d <= 243)) or (latitude < 0 and (d <= 59 or d >= 334)):  # Sommer
        base_turbidity = 1.5                                                                   # was: 2.0
      else:                                                                                    # Übergangszeit
        base_turbidity = 2.0                                                                   # was: 2.5
      m = kasten_young_airmass(90.0 - el_deg) if el_deg > 0 else 10
      linke_tl = base_turbidity + (m - 1) * 0.3
  
    # TOA (extraterrestrisch) & Clear-Sky
    E0   = pvpmc_E0(d)
    dni0 = solar_constant * E0
    ghi_toa = 0.0 if el_deg <= 0.0 else dni0 * cz
  
    if el_deg <= 0.0:
      ghi_cs = 0.0
    else:
      z_deg  = 90.0 - el_deg
      m_rel  = kasten_young_airmass(z_deg)
      m_abs  = m_rel * (pressure_from_alt(altitude) / 101325.0)
      delta_r = rayleigh_optical_thickness_factor(m_abs)
      T_TL  = math.exp(-0.8662 * linke_tl * m_abs * delta_r)
      ghi_cs  = dni0 * cz * T_TL

    # Baustelle
    d_out.update({"dni0": str(round(dni0,2))})
    d_out.update({"linke_tl": str(round(linke_tl,2))})
    d_out.update({"cz": str(round(cz,2))})
    d_out.update({"m": str(round(m,2))})
    if el_deg > 0:                                             # Baustelle!
      d_out.update({"T_TL": str(round(T_TL,2))})
    
    d_out.update({"theomaxsr": str(round(ghi_cs,2))})                        # momentane Globalstrahlung (Horizontal, Clear-Sky) [W/m²]
    d_out.update({"theosolar": str(round(ghi_toa,2))})                       # momentane TOA-GHI (ohne Dämpfung) [W/m²]
    #d_out.update({"dni_toa_wm2": str(round(dni0,2))})                        # momentane TOA-DNI (Top of Atmosphere-Direct Normal Irradiance normal zur Sonne) [W/m²] - 1400 W/m²
    d_out.update({"sunheight": str(round(el_deg,2))})                        # Sonnenhöhe [°]
    d_out.update({"sundir": str(round(az_deg,2))})                           # Sonnenazimut [°] (0=N, 90=E, 180=S, 270=W)
    d_out.update({"solarza": str(round(cz,2))})                              # cos(θz) - solar zenith angle (SZA)
    #d_out.update({"solartime": str(round(lst_h,2))})                         # lokale Sonnenzeit (LST) [h]
    d_out.update({"sunrise": str(round(sr_ts))})                             # lokaler UNIX-Timestamp für Sonnenaufgang (oder None)
    d_out.update({"sunset": str(round(ss_ts))})                              # lokaler UNIX-Timestamp für Sonnenuntergang (oder None)
    #d_out.update({"utc_offset": str(utc_offset_hoursours(local_timestamp))}) # aktueller UTC-Offset [h]
    d_out.update({"daydeg": str(round(math.degrees(delta),2))})              # Tages-Deklination [°]
    d_out.update({"turbidity": str(round(linke_tl,2))})                      # verwendeter Linke-Turbiditätsfaktor (Parameter)
    #d_out.update({"solar_constant": str(solar_constant)})                    # verwendete Solarkonstante [W/m²] (Parameter)
    ...
    debugPrint("solarInfo except: "+str(e))
    tprint("solarInfo except: "+str(e))
    pass
  return d_out                                                 # solarInfo2

def WeatherCondition(d, type = ""):                            # generate clientraw & Awekas condition/icon
  # v0.10: Awekas Current weather report conditions - partly
  # still missing: 3, 7, 15, 19, 21, 22, 23, 24, 26, 27, 28, 29, 30, 34
  ignoreKeys = {}
  condition = 0
  Awekas   = True if type.upper() == "AWEKAS" else False
  rr_num   = floatFallback(getfromDict(d,["rainratemm"],ignoreKeys),0)
  rr_num   = floatFallback(getfromDict(d,["rrain_piezo"],ignoreKeys),0) if rr_num == 0 else rr_num
  ws_num   = floatFallback(getfromDict(d,["windspeedkmh"],ignoreKeys),0)
  sr_num   = floatFallback(getfromDict(d,["solarradiation"],ignoreKeys),-1)
  temp_num = floatFallback(getfromDict(d,["tempc"],ignoreKeys),0)
  dew_num  = floatFallback(getfromDict(d,["dewptc"],ignoreKeys),0)
  is_snow  = True if (temp_num+dew_num)/2 < 2 and rr_num > 0 and rr_num >1  else False
  is_lsnow  = True if (temp_num+dew_num)/2 < 2 and rr_num > 0 and rr_num <=1 else False
  theosunsr   = round(theoreticalSun(int(time.time())),2)
  #perc_sr  = round(float(sr_num)*100/(theosunsr/float(SUN_COEF)),1) if theosunsr > 0 else 0
  perc_sr  = round(float(sr_num)*100/theosunsr,1) if theosunsr > 0 else 0
  cloudm   = floatFallback(getfromDict(d,["cloudm"],ignoreKeys),-1)

  if inTSWarning and ws_num < 25 and theosunsr > 0: condition = 19 if Awekas else 31       # 31 = thunderstorm, day
  elif inTSWarning and ws_num < 25 and theosunsr == 0: condition = 19 if Awekas else 17    # 17 = thunderstorm, night
  elif inTSWarning and ws_num >= 25: condition = 19 if Awekas else 35                      # 35 = thunderstorm with storm
  elif ws_num >= 50: condition = 20 if Awekas else 32                                      # 32 = tornado
  elif ws_num >= 25: condition = 20 if Awekas else 33                                      # 33 = storm
  elif is_snow and theosunsr > 0: condition = 14 if Awekas else 25                         # 25 = snow, day
  elif is_snow and theosunsr == 0: condition = 14 if Awekas else 16                        # 16 = snow, night
  elif is_lsnow: condition = 13 if Awekas else 27                                          # 27 = light snow/snow showers
  elif rr_num > 0 and rr_num <= 1: condition = 23 if Awekas else 10                        # 10 = drizzle
  elif rr_num > 0 and rr_num < 2.5: condition = 10 if Awekas else 20                       # 20 = light rain
  elif rr_num > 0 and rr_num < 10 and theosunsr > 0: condition = 11 if Awekas else 20      # 20 = rain, day
  elif rr_num > 0 and rr_num < 10 and theosunsr == 0: condition = 11 if Awekas else 14     # 14 = rain, night
  elif rr_num > 0 and rr_num >= 10 and theosunsr > 0: condition = 12 if Awekas else 8      #  8 = heavy rain, day
  elif rr_num > 0 and rr_num >= 10 and theosunsr == 0: condition = 12 if Awekas else 12    # 12 = heavy rain, night
  elif temp_num-dew_num <= 0.0 and theosunsr > 0: condition = 7 if Awekas else 6           #  6 = fog day
  elif temp_num-dew_num <= 0.0 and theosunsr == 0: condition = 7 if Awekas else 11         # 11 = fog night
  elif not Awekas and theosunsr > 0:                                                       # day
    if perc_sr > 120: condition = 0                                                        #  0 = sunny, day (=0=28=32)
    elif perc_sr > 80: condition = 9                                                       #  9 = mainly fine, meist sonnig (=5=9)
    elif perc_sr > 60: condition = 5                                                       #  5 = dry (cloudy periods) ganz wenige Wolken, day (=5=9)
    elif perc_sr > 40: condition = 2                                                       #  2 = cloudy, wenige Wolken, day (=2=3=19)
    elif perc_sr > 20 and cloudm > 400: condition = 2                                      #  2 = cloudy, wenige Wolken, day (=2=3=19)
    else: condition = 18                                                                   # 18 = overcast, wolkig, day (=13=18=34)
  elif not Awekas and theosunsr == 0:                                                      # night
    if cloudm > 1000: condition = 1                                                        #  1 = clear, klar, night
    elif cloudm > 500: condition = 4                                                       #  4 = cloudynight, wenige Wolken, night
    else: condition = 13                                                                   # 13 = overcast, wolkig, night (=13=18=34)
  else: condition = 0                                                                      # clear warning (Awekas)
  #tprint("WeatherCondition - time:  " + str(int(time.time()))  + " sr:     " + str(round(sr_num,2))         + " theosunsr:  " + str(theosunsr)         + "   %: " + str(perc_sr) + "%   --> condition: " + str(condition))
  return str(condition)                                        # WeatherCondition
...
def convertDictToMetricDict(d_e,IGNORE_EMPTY=True,LOX_TIME=True):
  ...
  ignoreValues = ["-9999","None","null","",None,"--"]
  ...
  for key,value in d_e.items():
    ...
    # v0.10 new keys
    elif "last24hrainin" in key: d_m.update({key.replace("rainin","rainmm") : intomm(value,2)})
    ...
    # Ambient-specific keys
      ...
      last_FWver = value
      d_m.update({key : value})
    # v0.09 WS90 compatibility - convert WS90 rain values to metric values - covers also last24hrain_piezo
    elif "rain_piezo" in key and key != "srain_piezo": d_m.update({key.replace("rain_piezo","rain_piezomm") : intomm(value,2)})
    # debug! - muss wieder raus!
    ...
    elif "co2x8z_tempf" in key:
      d_m.update({key.replace("tempf","tempc") : ftoc(value,1)})
    elif "vpd" in key:                                         # convert inHg to hPa
      d_m.update({key : inhgtokpa(value,2)})
    # v0.10: bgt = Black Global Temperature, wbgt = Wet Bulb Global Temperature
    elif key == "bgt" or key == "wbgt": d_m.update({key+"c" : ftoc(value,1)})
    # v0.10 WN52 - 16 channel soil temperature
    elif "soil_ec_temp" in key and (len(key) == 13 or len(key) == 14): d_m.update({key+"c" : ftoc(value,1)})
    else:
      d_m.update({key : value})
  ...
  if SENSOR_WARNING:
    ...
    for i in range(len(senmand_arr)):
      if getfromDict(d_e,[senmand_arr[i]]) == "null":
        missingSensor = True
        SensorIsMissed += senmand_arr[i] + sep
        #break
    if len(SensorIsMissed) >= sep_len and SensorIsMissed[-sep_len:] == sep: SensorIsMissed = SensorIsMissed[:-sep_len]
    if missingSensor:
  ...
  # new in v0.06 - battery-warning - modified in v0.10: now show and save the corresponding sensor names
  if BATTERY_WARNING:
    global inBatteryWarning, BattWarnSensor
    BattWarnSensor = checkBattery(d_m,last_FWver,battex_arr,",")
    if BattWarnSensor != "":
      if not inBatteryWarning:
        logPrint("<WARNING> battery level for sensor(s) " + BattWarnSensor + " is critical - please swap battery")
        sendUDP("SID=" + defSID + " batterywarning=1 critical=" + BattWarnSensor + " time=" + str(loxTime(time.time())))
        pushPrint("<WARNING> battery level for sensor(s) " + BattWarnSensor + " is critical - please swap battery")
        inBatteryWarning = True
        config.set("Status","inBatteryWarning",str(inBatteryWarning))
        config.set("Status","BattWarnSensor",BattWarnSensor)
        haveToSave = True
      ...
      config.remove_option("Status","BattWarnSensor")
      haveToSave = True
      inBatteryWarning = False
  ...
  if CO2_WARNING:
    global inCO2Warning
    co2 = getfromDict(d_m,CO2_WARNKEY)                         # from config file - default: ["co2","co2in","co2_in","co2_in_aqin"]
    try:
      co2_num = float(co2)
...
def forwardDictToMeteoTemplate(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys):
  ...
  for key,value in d.items():
    ...
    elif "soilmoisture" in key and (len(key) == 13 or len(key) == 14):
      outstr += "SM" + str(key[12:]) + "=" + str(value) + "&"  # v0.10 support for soilmoisture10..16
    elif "soil_ec_hum" in key and (len(key) == 12 or len(key) == 13):
      outstr += "SM" + str(key[11:]) + "=" + str(value) + "&"  # v0.10 support for WN52/soilmoisture10..16
    ...
    # battery data - send OK or LOW
      ...
      outstr += "WBAT=" + battval + "&"
    elif key == "wh80batt" or key == "wh85batt" or key == "wh90batt":
      ...
      battval = "OK" if value == "0" else "LOW"
      outstr += "LBAT=" + battval + "&"
    elif "soilbatt" in key and (len(key) == 9 or len(key) == 10):
      battval = "OK" if float(value) > 1.2 else "LOW"
      outstr += "SM" + str(key[8:]) + "BAT=" + battval + "&"
    elif "soil_ec_batt" in key and (len(key) == 13 or len(key) == 14):
      battval = "OK" if float(value) > 1.2 else "LOW"
      outstr += "SM" + str(key[8:]) + "BAT=" + battval + "&"
    ...
    elif "battsm" in key and len(key) == 8:
      battval = "OK" if value == "1" else "LOW"                # should be ok
      outstr += "SM" + str(key[6]) + str(key[7]) + "BAT=" + battval + "&"
    elif key == "batt_25":
      battval = "OK" if value == "1" else "LOW"                # should be ok
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      ...
      ret = str(r.status_code)
      okstr = "<ERROR> " if r.status_code not in range(200,205) else ""
...
def forwardDictToWC(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys):
  ...
  for key,value in d.items():
    ...
    elif "soilmoisture" in key and len(key) == 14:             # v0.10 support for soilmoisture10..16
      outstr += "soilmoist" + str(key[12:]) + "=" + str(value) + "&"
    elif "soil_ec_hum" in key and len(key) == 12:
      if key[11] == "1":
        outstr += "soilmoist" + "=" + str(value) + "&"
      else:
        outstr += "soilmoist" + "0" + str(key[12]) + "=" + str(value) + "&"
    elif "soil_ec_hum" in key and len(key) == 13:             # v0.10 support for WN52/soilmoisture10..16
      outstr += "soilmoist" + str(key[12:]) + "=" + str(value) + "&"
    ...
    elif "soilhum" in key and len(key) == 9:                   # v0.10 support for soilhum10..16
      outstr += "soilmoist" + str(key[7:]) + "=" + str(value) + "&"
    ...
    elif key == "co2" or key == "co2in" or key == "co2_in" or key == "co2_in_aqin":
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      ...
      if USER_AGENT != "": headers.update({"User-Agent": USER_AGENT})
...
def forwardDictToUDP(url,d_in,fwd_sid,fwd_pwd,status,script,nr,ignoreKeys,remapKeys,sep=" "):
  ...
  if status:
    ...
    bw_what = sep+"battwarn=\"" + BattWarnSensor + "\"" if inBatteryWarning and BattWarnSensor != "" else ""
    outstr += sep+"running=" + str(int(wsconnected)) + sep + "wswarning=" + str(int(inWStimeoutWarning)) + sep + "sensorwarning=" + str(int(inSensorWarning)) + sw_what + sep +"batterywarning=" + str(int(inBatteryWarning)) + bw_what + sep + "stormwarning=" + str(int(inStormWarning)) + sep + "tswarning=" + str(int(inTSWarning)) + sep + "updatewarning=" + str(int(updateWarning)) + sep + "leakwarning=" + str(int(inLeakageWarning)) + sep + "co2warning=" + str(int(inCO2Warning)) + sep + "intvlwarning=" + str(int(inIntervalWarning))
  if script != "":
    outstr = modExec(nr, script, outstr)                       # modify outstr with external script before sending
    if outstr == execOnly:                                     # just run the exec-script but do not forward the string
      updateFWDstate(execOnly, nr)
...
def isBlacklisted(key):                                        # blacklist for spread, signal
  ...
  return bool((key == "spread" or key == "spreadin" or (key[:6] == "spread" and len(key) == 7) or key == "spread_co2" or key == "radcompensation" or key == "newVersion" or key == "upgrade" or key == "rainFallPriority" or key == "rainGain" or key == "piezo" or key == "rstRainDay" or key == "rstRainWeek" or key == "rstRainYear" or key == "rain1_gain" or key == "rain2_gain" or key == "rain3_gain" or key == "rain4_gain" or key == "rain5_gain") or (key[:2] == "wh" and key.find("sig") > 0))


def dictToWU(d, ignoreKeys):
  outstr = ""
  dontuse = ("PASSKEY","PASSWORD","ID","model","freq")
  ignoreValues = ["-9999","None","null","",None,"--"]
  ...
  for key,value in d.items():
    ...
    elif "temp" in key and len(key) == 6 and key[-1] == "f" and "PASSKEY" in d.keys():
    ...
    elif "humidity" in key and len(key) == 9 and "PASSKEY" in d.keys():
    ...
    # v0.10 use WH45 data only if no WH41/WH43 #1 present
      ...
      outstr += "AqPM10=" + str(value) + "&"
      """
    elif (key == "co2" or key == "co2in" or key == "co2_in" or key == "co2_in_aqin") and not ("pm25_ch1" in d.keys() or "pm25" in d.keys()):
      outstr += "AqCO2=" + str(value) + "&"                    # no official WU key!
    else:                                                      # all other values will be sent as present
      outstr += key + "=" + str(value) + "&"
  if len(outstr) > 0 and outstr[-1] == "&": outstr = outstr[:-1]
  return outstr                                                # dictToWU
...
def forwardStringToWindy(url,payload,fwd_sid,fwd_pwd,status,script,nr,ignoreKeys,remapKeys):
  # convert incoming string to WU format and send to Windy.com v2 API via GET
  # new Windy API: https://stations.windy.com/pws/v2/observation/update with PASSWORD and id params
  debugPrint("forwardStringToWindy "+nr+" start")
  d = remappedDict(stringToDict(payload,"&"),remapKeys,nr)     # remap keys in current dictionary
  if status: d.update(addStatusToDict(d, True))                # append status to the dict d if set
  outstr = dictToWU(d, ignoreKeys)
  if script != "":
    ...
    if outstr == execOnly:                                     # just run the exec-script but do not forward the string
      updateFWDstate(execOnly, nr)
      return
  # build Windy v2 API URL with station password and id
  windy_url = url
  if "?" not in windy_url:
    windy_url += "?"
  elif not windy_url.endswith("&") and not windy_url.endswith("?"):
    windy_url += "&"
  if fwd_pwd != "":
    windy_url += "PASSWORD=" + str(fwd_pwd) + "&"
  if fwd_sid != "":
    windy_url += "id=" + str(fwd_sid) + "&"
  ret = ""
  okstr = "<ERROR> "
  v = 0
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      r = requests.get(windy_url+outstr,timeout=httpTimeOut)
      ret = str(r.status_code)
      okstr = "<ERROR> " if r.status_code not in range(200,205) else ""
      if r.status_code in range(400,500): v = 400
    except requests.exceptions.Timeout as err:
  ...
  updateFWDstate(code, nr)
  tries = "" if v == 1 or v > httpTries else " ("+str(v)+" tries"+qstr+")"
  if sndlog: sndPrint(okstr + "FWD-"+nr+": " + windy_url + outstr + " : " + ret + tries)
  debugPrint("forwardStringToWindy "+nr+" stop")
  return                                                       # forwardStringToWindy
...
def convBattToAMB(key, value):
  ...
  elif ("wh80batt" in key or "wh90batt" in key or "wh85batt" in key) and float(value) > 2.3: battok = "1"
  elif "ldsbatt" in key and float(value) > 2.41: battok = "1"
  return battok
...
def forwardStringToAMB(url,payload,fwd_sid,fwd_pwd,status,script,nr,ignoreKeys,remapKeys):
  ...
  for key,value in d.items():
    ...
    # exchange some keys with Ambient-keys
      ...
      outstr += key.replace("soil_ec_hum","soilhum")  + "=" + str(value) + "&"
    elif "leak_ch" in key:
    ...
    elif key == "co2" or key == "co2in" or key == "co2_in":
      outstr += "co2_in_aqin=" + str(value) + "&"
    elif key == "co2_24h" or key == "co2in_24h":
    ...
    elif key == "wh65batt" or key == "wh80batt" or key == "wh90batt" or key == "wh85batt":
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      ...
      ret = str(r.text) if r.status_code in range(200,205) else str(r.status_code)
      okstr = "<ERROR> " if r.status_code not in range(200,205) or ret != "OK" else ""
      if r.status_code in range(400,500): v = 400
    except requests.exceptions.Timeout as err:
      ret = "TIMEOUT"
    except requests.exceptions.ConnectionError as err:
...
def dictToWUServer(d_in, sep, metric, pws = "", apikey = ""):  # if pws = FOSHKplugin and key = MMMtrigger ("MMM") send additional keys: indoorTemp, indoorHumidity, lightningTime, lightningCount, lightningDistance, AqPM10, AqCO2, AqPM1, AqPM4
  s = ""
  d = d_in.copy()

  # some more needed fields
  ignoreValues = ["-9999","None","null","",None,"--"]
  missing = ["wnowcnd","theosunsr","theouv","aqtime","starttime"] # some more missing keys from last_d_all
  for key in missing:
    value = getfromDict(last_d_all,[key])
    if value not in ignoreValues: d.update({key:str(value)})
  # add sun data
  d.update(sunDict)
  ...
  if d != "":
    ...
    if wert == "null": wert = prgname+" "+prgbuild
    s += "\"softwareType\":\""+ wert +"\","
    ...
    if wert != "null":
      s += "\"AqPM2.5\":"+ wert + ","
    # same for PM10
    wert = getfromDict(d,["pm10_ch1","pm10_co2","AqPM10"])
    if wert != "null":
      s += "\"AqPM10\":"+ wert + ","
    # same for PM4 (WH46)
    wert = getfromDict(d,["pm4_ch1","pm4_co2","AqPM4"])
    if wert != "null":
      s += "\"AqPM4\":"+ wert + ","
    # same for PM1 (WH46)
    wert = getfromDict(d,["pm1_ch1","pm1_co2","AqPM1"])
    if wert != "null":
      s += "\"AqPM1\":"+ wert + ","
    ...
    # v0.10: WN52 compatibility
    for i in range(1,17):
      try:
        i_s = str(i)
        soil = getfromDict(d,["soil_ec_hum"+i_s])
        if soil != "null":
          if i == 1:
            s += "\"soilmoisture\":"+ soil +","
          else:
    ...
    if pws == prgname and apikey == MMMtrigger:
      s += "\"indoorHumidity\":"+ getfromDict(d,["humidityin"])+","
      wert = getfromDict(d,["lightning_time"])
      if wert != "null": s += "\"lightningTime\":"+wert+","
      wert = getfromDict(d,["lightning_num","lightning_day"])
      if wert != "null": s += "\"lightningCount\":"+wert+","
      wert = getfromDict(d,["co2_ch1","co2","co2_in_aqin","AqCO2"])
      if wert != "null":  s += "\"AqCO2\":"+ wert + ","
      wert = getfromDict(d,["srain_piezo"])
      if wert != "null":  s += "\"isRaining\":"+ wert + ","
      wert = getfromDict(d,["aqtime"])
      if wert != "null":  s += "\"aqTime\":"+ wert + ","
      s += "\"realtimeFrequency\":"+ getfromDict(d,["isintvl10","isintvl"])+","
      for i in range(1,5):
        try:
          i_s = str(i)
          leak = getfromDict(d,["leak_ch"+i_s,"leak"+i_s])
          if leak != "null": s += "\"leakAlarm" + i_s + "\":"+ leak + ","
        except ValueError: pass
      wert = getfromDict(d,["sunhours"])
      if wert != "null":  s += "\"sunHours\":"+ wert + ","
      wert = getfromDict(d,["sunrise"])
      if wert != "null":  s += "\"sunRise\":"+ wert + ","
      wert = getfromDict(d,["sunset"])
      if wert != "null":  s += "\"sunSet\":"+ wert + ","
      wert = getfromDict(d,["sunheight"])
      if wert != "null":  s += "\"sunHeight\":"+ wert + ","
      wert = getfromDict(d,["sundir"])
      if wert != "null":  s += "\"sunDir\":"+ wert + ","
      wert = getfromDict(d,["theomaxsr"])
      if wert != "null":  s += "\"theomaxSR\":"+ wert + ","
    ...
    alt = getfromDict(d,["elev"])
    if alt == "null" and COORD_ALT != "": alt = COORD_ALT      # exchange defaults with given parameters
    if not metric: alt = mtofeet(alt,3)
    ...
    if pws == prgname and apikey == MMMtrigger:
      s += "\"indoorTemp\":"+ getfromDict(d,["tempinc","tempinf","indoortempc","indoortempf"])+","
      distance = getfromDict(d,["lightning","lightning_distance"])
      if not metric and distance != "null": distance = kmtomi(distance,4)
      s += "\"lightningDistance\":"+ str(distance)+","
      s += "\"precipDay\":"+ getfromDict(d,["dailyrainin","dailyrainmm"])+","
      s += "\"precipWeek\":"+ getfromDict(d,["weeklyrainin","weeklyrainmm"])+","
      s += "\"precipMonth\":"+ getfromDict(d,["monthlyyrainin","monthlyrainmm"])+","
      s += "\"precip24\":"+ getfromDict(d,["last24hrainin","last24hrainmm"])+","
      s += "\"precipYear\":"+ getfromDict(d,["yearlyrainin","yearlyrainmm"])+","
      s += "\"precipEvent\":"+ getfromDict(d,["eventrainin","eventrainmm"])+","
      s += "\"piezoRate\":"+ getfromDict(d,["rrain_piezo","rrain_piezomm"])+","
      s += "\"piezoDay\":"+ getfromDict(d,["drain_piezo","drain_piezomm"])+","
      s += "\"piezoWeek\":"+ getfromDict(d,["wrain_piezo","wrain_piezomm"])+","
      s += "\"piezoMonth\":"+ getfromDict(d,["mrain_piezo","mrain_piezomm"])+","
      s += "\"piezo24\":"+ getfromDict(d,["last24hrain_piezo","last24hrain_piezomm"])+","
      s += "\"piezoYear\":"+ getfromDict(d,["yrain_piezo","yrain_piezomm"])+","
      s += "\"piezoEvent\":"+ getfromDict(d,["erain_piezo","erain_piezomm"])+","
      s += "\"windRun\":"+ getfromDict(d,["windrun","windrunkm"])+","

      # feelslikec feelslikef humi_co2 leafwetness_ch1 leak_ch1 maxdailygust maxdailygustkmh pm10_co2 pm25_co2 rtime runtime sunhours tc_co2 tf_co2 windrunkm windrun windspeedkmh windspeedmph 
    s += "\"elev\":"+ alt +""
    s += "}"
    s += "}]}"
...
def remappedString(s_in,remapKeys):
  d = stringToDict(s_in,"&")
  for key, value in remapKeys.items():
    try:
      newval = last_d_all[value[1:]] if value[0] == "@" else value      # use value of dict if @ given
      ...
      pass
  try:
    s_out = dictToString(d,"&",klammern=False,ignoreKeys={},ignoreValues={},withkey=True,withvalue=True,hideSpace=False)
  except: s_out = s_in
  return s_out
...
def dictToWeeWX(d_in,nr,ignoreKeys,remapKeys):
  ...
  outstr += getfromDict(d,["soilmoisture1","soil_ec_hum1","soilmoisture"],ignoreKeys).replace(".",",") + ";" # 29    idMoisture1
  outstr += getfromDict(d,["soilmoisture2","soil_ec_hum2"],ignoreKeys).replace(".",",") + ";"                # 30    idMoisture2
  outstr += getfromDict(d,["soilmoisture3","soil_ec_hum3"],ignoreKeys).replace(".",",") + ";"                # 31    idMoisture3
  outstr += getfromDict(d,["soilmoisture4","soil_ec_hum4"],ignoreKeys).replace(".",",") + ";"                # 32    idMoisture4
...
def forwardDictToAwekas(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys,fwd_options):
  # use API to upload data to Awekas
  debugPrint("forwardDictToAwekas "+nr+" start")
  # additional options
  o = stringToDict(fwd_options,",",strip=True)
  snowsensor = getfromDict(o,["SNOW_DEPTH","SNOW_HEIGHT","snow"],ignoreKeys,SNOW_DEPTH)
  d = remappedDict(d_in,remapKeys,nr)                          # remap keys in current dictionary
  ignoreValues = ["-9999","None","null","",None,"--"]
  ...
  # for all soil moisture sensors (1..16)
  for i in range(1,17):
    try:
      i_s = str(i)
      value = getfromDict(d,["soilmoisture"+i_s,"soil_ec_hum"+i_s,"soilhum"+i_s],ignoreKeys)
  ...
  # Baustelle
  oc = str(condition)                                          # original condition
  nc = WeatherCondition(d, type = "Awekas")                    # new condition
  spread = floatFallback(getfromDict(d,["spread"],ignoreKeys),0)
  if oc != nc and HIDDEN_FEATURES:
    tprint("<DEBUG> old condition: " + oc +" new: "+ nc +" rr: "+str(rr_num)+" lv: "+str(lv_num)+" spread: "+str(spread))
    pushPrint("<DEBUG> old condition: " + oc +" new: "+ nc +" rr: "+str(rr_num)+" lv: "+str(lv_num)+" spread: "+str(spread))

  # warning condition
  outstr += ";"

  # snow height in cm (comes in mm)
  value = getfromDict(d,[snowsensor])                          # defaults to depth_ch1, can be overridden with option SNOW
  snow = floatFallback(value)
  snow = snow / 10 if not (IGNORE_EMPTY and value in ignoreValues) else "null"
  outstr += str(snow) + ";" if not (IGNORE_EMPTY and value in ignoreValues) else ";"
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      ...
      ret = str(r.text) if r.status_code in range(200,205) else str(r.status_code)
      okstr = "<ERROR> " if r.status_code not in range(200,205) or "OK" not in ret else ""
...
def forwardDictToWindguru(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys):
  # convert incoming metric dict to Windguru-API via http/GET - see https://stations.windguru.cz/upload_api.php
  debugPrint("forwardDictToWindguru "+nr+" start")
  ignoreValues = ["-9999","None","null","",None,"--"]
  # remap Keys
  d = remappedDict(d_in,remapKeys,nr)                                                    # remap keys in current dictionary
  now = time.localtime()
  if url == "": url = "http://www.windguru.cz/upload/api.php"                            # fallback to default URL
  uid = fwd_sid                                                                          # UID of your station = unique string you choosed during station registration
  #unixtime = ""                                                                         # time of measurement as unix timestamp (epoch)
  datetime = time.strftime('%Y-%m-%dT%H:%M:%S%z', now)                                   # time of measurement in ISO 8601 format (example: 2025-10-02T09:08:42+02:00) (in case you send both unixtime and datetime, unixtime will take precedence)
  #interval = ""                                                                         # seconds, archive interval over which wind information is averaged
  wind_avg = kmhtokts(getfromDict(d,["windspeedkmh"],ignoreKeys),1)                      # knots, average wind speed during interval
  wind_max = kmhtokts(getfromDict(d,["windgustkmh"],ignoreKeys),1)                       # knots, max wind speed during interval
  #wind_min                                                                              # min wind speed during interval
  wind_direction = getfromDict(d,["winddir"],ignoreKeys)                                 # degrees, average wind direction during interval
  temperature = getfromDict(d,["tempc"],ignoreKeys)                                      # celsius
  rh = getfromDict(d,["humidity"],ignoreKeys)                                            # humidity, percent
  mslp = getfromDict(d,["baromrelhpa","baromhpa"],ignoreKeys)                            # hPa, mean sea level pressure
  precip = getfromDict(d,["hourlyrainmm","rainmm","hrain_piezomm"],ignoreKeys)           # precipitation in milimeters (not displayed anywhere yet, but API is ready to accept)
  #precip_interval                                                                       # seconds, archive interval over which precipitation is accumulated in a sum - defaults to 3600

  # outstr basteln
  salt = time.strftime('%Y%m%dT%H%M%S', now)                                             # any random string, should change with every upload request (you can use current timestamp for example...)
  hashstr = salt+uid+fwd_pwd
  ghash = hashlib.md5(hashstr.encode('utf-8')).hexdigest()                               # salt+uid+password
  outstr = "?uid="+uid+"&salt="+salt+"&hash="+ghash
  if wind_avg not in ignoreValues: outstr += "&windavg="+wind_avg
  if wind_max not in ignoreValues: outstr += "&windmax="+wind_max
  if wind_direction not in ignoreValues: outstr += "&wind_direction="+wind_direction
  if temperature not in ignoreValues: outstr += "&temperature="+temperature
  if rh not in ignoreValues: outstr += "&rh="+rh
  if mslp not in ignoreValues: outstr += "&mslp="+mslp
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Connection': 'Close'}
      if USER_AGENT != "": headers.update({"User-Agent": USER_AGENT})
      debugPrint("forwardDictToWindguru "+nr+" URL: "+url+outstr)
      r = requests.get(url+outstr,headers=headers,timeout=httpTimeOut)
      ret = str(r.status_code)
      debugPrint("forwardDictToWindguru "+nr+" ret: "+ret)
      okstr = "<ERROR> " if r.status_code not in range(200,205) else ""
      if r.status_code in range(400,500): v = 400
    except requests.exceptions.Timeout as err:
      ret = "TIMEOUT"
  ...
  tries = "" if v == 1 or v > httpTries else " ("+str(v)+" tries)"
  if sndlog: sndPrint(okstr + "FWD-"+nr+": " + url + outstr + " : " + ret + tries)
  debugPrint("forwardDictToWindguru "+nr+" stop")
  return                                                       # forwardDictToWindguru
...
def forwardDictToWeather365(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys):
  ...
  for key,value in d.items():
    if key in ignoreKeys or key in dontuse or (IGNORE_EMPTY and value in ignoreValues):
      ...
      outstr += "soilmoisture" + str(key[12:]) + "=" + str(value) + "&"
    elif "soil_ec_hum" in key and len(key) == 12:             # sensor1=5cm, 2=10/15cm, 3=20-30cm, 4=40-50cm
      if key[11] == "1":
        outstr += "soilmoisture=" + str(value) + "&"
      else:                                                    # all sensors are for different depth 5, 10, 20, 50 cm
        outstr += "soilmoisture" + str(key[11]) + "=" + str(value) + "&"
    elif "soil_ec_hum" in key and len(key) == 13:             # v0.10 support for WN52/soilmoisture10..16
      outstr += "soilmoisture" + str(key[11:]) + "=" + str(value) + "&"
    elif key == "UV" or key == "uv":
  ...
  if wdir10m != "":                                            # prefer 10m-average to winddir if available
    ...
    outstr += "winddir=" + wdir + "&"
  # coordinates
  if lat == "": lat = COORD_LAT                                # add coordinates if available, prefer parameter
...
def dictToCLIENTRAW(d_in,nr,ignoreKeys,remapKeys):
  ...
  # create & initialize the output array
  a = []
  for i in range(178): a.append("--")                          # perhaps "NULL"

  # for temp (90-99) - same for wind (80-89), rain (100-109), winddir (146-155) - see https://github.com/mapkyca/WD-clientraw-parse
  global last_sixtime
  global lasthour
  if last_sixtime == 0 or time.time() - last_sixtime >= 360:   # every 6 minutes (360 seconds)
    if last_sixtime != 0:
      #arr = [temp, rainrate, windspd, winddir]
      lasthour.append([getfromDict(d,["tempc"],ignoreKeys), getfromDict(d,["rainratemm"],ignoreKeys), kmhtokts(getfromDict(d,["windspeedkmh"],ignoreKeys),1), getfromDict(d,["winddir"],ignoreKeys)])
      debugPrint("saved current lasthour data: last: "+str(last_sixtime)+" diff: "+str(time.time() - last_sixtime))
    last_sixtime = time.time()
  ...
  a[11]  = getfromDict(min_max,["rainratemm_max"],ignoreKeys)
  a[12]  = getfromDict(d,["tempinc"],ignoreKeys)
  a[13]  = getfromDict(d,["humidityin","indoorhumidity"],ignoreKeys)
  a[14]  = getfromDict(d,["tf_ch1c"],ignoreKeys)                         # soiltemp
  ...
  icon = str(WeatherCondition(d))
  #icon = str(WeatherCondition(last_d_all))
  a[48]  = icon
  ...
  # 80 - 89 windspeed 60/54/48/42/36/30/24/18/12/6 min                   # arr = [temp, rainrate, windspd, winddir]
  for i in range(80,90): a[i] = lasthour[i-80][2]

  # 90 - 99 outtemp 60/54/48/42/36/30/24/18/12/6 min
  for i in range(90,100): a[i] = lasthour[i-90][0]

  # 100 - 109 rain 60/54/48/42/36/30/24/18/12/6 min                      # is this really the rain rate?
  for i in range(100,110): a[i] = lasthour[i-100][1]
  ...
  # 146 - 155 winddir 60/54/48/42/36/30/24/18/12/6 min
  for i in range(146,156): a[i] = lasthour[i-146][3]

  try:                                                                   # leafwetness - just ch #1
    a[156] = str(round(float(leafTo15(getfromDict(d,["leafwetness_ch1","leafwetness1","leafwetness"],ignoreKeys)))))
  except: pass

  a[157] = getfromDict(d,["soilmoisture1","soil_ec_hum1"],ignoreKeys)    # soilmoisture - just ch #1 but also from WN52 #1
    ...
    try:
      ...
      if binary:
        r = requests.post(url, data=({
            ...
            "prgname":prgname,
            "prgver":prgver,
          }), files={'image': open(content, 'rb')}, timeout=httpTimeOut, headers=headers) # , headers={'Content-Type': 'application/octet-stream'}
      else:
        r = requests.post(url, data=({
            "user":user,
            "password":password,
            ...
            "prgver":prgver,
            "content":content
          }), timeout=httpTimeOut, headers=headers)
      #ret = str(r.status_code)
...
def dictToWSWin(d_in,nr,ignoreKeys,remapKeys):
  ...
  outstr += getfromDict(d,["humidity6"],ignoreKeys).replace(".",",") + ";"                                   # 24    idFeuchte7
  outstr += getfromDict(d,["soilmoisture1","soilmoisture","soil_ec_hum1"],ignoreKeys).replace(".",",") + ";" # 29    idMoisture1
  outstr += getfromDict(d,["soilmoisture2","soil_ec_hum2"],ignoreKeys).replace(".",",") + ";"                # 30    idMoisture2
  outstr += getfromDict(d,["soilmoisture3","soil_ec_hum3"],ignoreKeys).replace(".",",") + ";"                # 31    idMoisture3
  outstr += getfromDict(d,["soilmoisture4","soil_ec_hum4"],ignoreKeys).replace(".",",") + ";"                # 32    idMoisture4
  outstr += leafTo15(getfromDict(d,["leafwetness_ch1","leafwetness1","leafwetness"],ignoreKeys).replace(".",",")) + ";"     # 25    idLeafWet1
...
def forwardDictToMQTT(url,d_in,fwd_sid,fwd_pwd,status,script,nr,ignoreKeys,remapKeys,MQTTsendMin,fwd_options,metric):
  ...
  global last_mqtt
  global MQTTsendTime
  global IOT_MQTT_LEVEL
  MQTTsendAll = False
  # v0.10 - gather options from FWD_OPTION
  o = stringToDict(fwd_options.replace(r"\,","[Komma]"),",",strip=True)
  ...
  hass_dev_name = getfromDict(o,["devname"],ignoreKeys,"FOSHKplugin").replace("[Komma]",",")
  ...
  useCategories = mkBoolean(getfromDict(o,["diagnostic","categories"],ignoreKeys,"False"))
  dtime_format = getfromDict(o,["dtime_format"],ignoreKeys,DT_FORMAT).replace("[Komma]",",")
  # for hoetz
  withjson = mkBoolean(getfromDict(o,["json"],ignoreKeys,"False"))
  withkeyvalue = mkBoolean(getfromDict(o,["keyvalue"],ignoreKeys,"True"))
  # enable "--" as value (for Werner)
  allowdmin = mkBoolean(getfromDict(o,["allowdmin"],ignoreKeys,"False"))
  # ignoreKeys = blacklist keys; ignoreValues = empty values
  ignoreValues = ["-9999","None","null","",None] if allowdmin else ["-9999","None","null","",None,"--"]
  # v0.10: qos & retain
  mqtt_qos = getfromDict(o,["qos"],ignoreKeys,"1").replace("\"","")
  mqtt_qos = "1" if not (mqtt_qos == "0" or mqtt_qos == "1" or mqtt_qos == "2") else mqtt_qos
  mqtt_retain = getfromDict(o,["retain"],ignoreKeys,"true").replace("\"","")
  mqtt_retain = "true" if not (mqtt_retain == "true" or mqtt_retain == "false") else mqtt_retain
  ...
  d.update(iotDict)                                            # IoT

  d.update(sunDict)                                            # additional sun values
  ...
  # for hoetz - modified: follow ignoreKeys & allowdmin also for json topic
  if withjson:
    d_json = {}                                                # empty output array to be filled
    for key,value in d.items():
      if key in ignoreKeys or (IGNORE_EMPTY and value in ignoreValues) or value == "": None
      elif not MQTTonChangeOnly or MQTTsendAll or (MQTTonChangeOnly and (key not in last_mqtt or value != last_mqtt[key])): d_json.update({key:value})
    #tprint(nr+" "+str(MQTTonChangeOnly)+" "+str(MQTTsendAll)+" "+json.dumps(d_json))
    d_out.append({'topic':level + "/"+prefix+"json",'payload': json.dumps(d_json)})

  if withkeyvalue:
    for key,value in d.items():
      if key in ignoreKeys or (IGNORE_EMPTY and value in ignoreValues) or value == "":
        None
        ...
        """
        # nur bei HA-Discovery oder generell?
  
        # im "normalen" dict herausnehmen, dafuer separaten MQTT-Topic anlegen
        if "-trigger" in key:
          #tprint(nr+": "+key+": "+str(value))
          hass_trigger_state = key.replace("trigger","ac_status") if key[4:6] == "sp" else key.replace("trigger","water_status")
          payload = { "name": key, "unique_id": uniqid+"-"+key, "cmd_t": IOT_MQTT_LEVEL+key, "stat_t": level + "/" + prefix+hass_trigger_state}
          payload.update({"ent_cat":"config"})
          payload.update({"pl_on":"1"})
          payload.update({"pl_off":"0"})
          payload.update({"qos":"1"})
          payload.update({"retain":"true"})
          tprint(payload)
          d_out.append({'topic':'homeassistant/switch/'+uniqid+'/'+key+'/config','payload': json.dumps(payload)})
          #tprint("1: "+str(payload))
        """
        ...
        if HAdiscovery:
          ...
          hass_state_cla = "measurement"
          hass_ent_cat = None                                    # default - see https://developers.home-assistant.io/blog/2021/10/26/config-entity/
          hass_display_precision = None
  
          if ("temp" in key or "tc_co2" in key or "tf_co2" in key or "tf_ch" in key or "dewpt" in key or "windchill" in key or "feelslike" in key or "heatindex" in key or ("bgt" in key and not "cat" in key and not "batt" in key)) and not "time" in key:
            hass_icon = "mdi:thermometer"
            hass_dev_cla = "temperature"
            hass_unit_of_meas = "°C" if metric else "°F"
          if "soil_ec_temp" in key:
            hass_icon = "mdi:thermometer"
            hass_dev_cla = "temperature"
            hass_unit_of_meas = "°C" if key[-1] == "c" else "°F"
          elif "spread" in key and not "time" in key:
            hass_icon = "mdi:delta"
            hass_unit_of_meas = "K"
          elif ("soilad" in key or "soil_ec_hum_ad" in key or "soil_ec_ad" in key) and not "time" in key:
            hass_icon = "mdi:watering-can-outline"
            hass_dev_cla = "aqi"
          elif ("humi" in key or "soilmoisture" in key or "soil_ec_hum" in key) and not "time" in key:
            hass_icon = "mdi:watering-can" if "soil" in key else "mdi:water-percent"
            hass_dev_cla = "humidity"
            hass_unit_of_meas = "%"
          elif "soil_ec" in key and (len(key) == 8 or len(key) == 9):
            hass_icon = "mdi:sprout"
            hass_dev_cla = "conductivity"
            hass_unit_of_meas = "µS/cm"
            ...
            if not "text" in key:
              hass_unit_of_meas = "°"
              hass_state_cla = "measurement_angle"
          ...
          elif ("time" in key or "dateutc" in key or "suncheck" in key or "minmax_init" in key) and not "realtime" in key:
            hass_icon = "mdi:clock"
            hass_state_cla = None
            if key == "runtime" or "run_time" in key:
              #hass_dev_cla = "duration"                         # shows hhhh:mm:ss
              hass_val_tpl = "{{ timedelta(seconds = " + value + ") }}"
              #hass_unit_of_meas = "s"
              ...
              #hass_val_tpl = "{{ as_local(as_datetime(value)) }}"  # show as local time = "2024-03-31 17:08:53+02:00" thus "%Y-%m-%d %H:%M:%S%z"
              hass_val_tpl = "{{ " + str(value) + " | " + "timestamp_custom(\"" + dtime_format + "\") }}"   # fallback: DT_FORMAT = "%d.%m.%Y %H:%M:%S"
            elif key == "dateutc":
              hass_ent_cat = "diagnostic"
          ...
          elif ("batt" in key and not "warning" in key) or "volt" in key:
            hass_ent_cat = "diagnostic"
            if "ac" in key:                                      # includes iot...ac_voltage
            ...
            elif value == "9": hass_icon = "mdi:battery-off"
              ...
              hass_icon = "mdi:battery"
          # Werner
          elif key == "radcompensation" or key == "newVersion" or key == "upgrade" or key == "rainFallPriority" or key == "rainGain" or key == "rstRainDay" or key == "rstRainWeek" or key == "rstRainYear" or key == "piezo" or key == "rain1_gain" or key == "rain2_gain" or key == "rain3_gain" or key == "rain4_gain" or key == "rain5_gain":
            hass_ent_cat = "diagnostic"
            ...
            if key == "srain_piezo":
              #hass_dev_cla = "precipitation"                    # needs a unit - so disable the class
              doNothing()
            elif "rainrate" in key or "rrain" in key:
              hass_dev_cla = "precipitation_intensity"
              hass_unit_of_meas = "mm/h" if metric else "in/h"
              hass_display_precision = "1"
            else:
          ...
          elif key == "co2" or key == "co2_in" or key == "co2_24h" or "co2in" in key:
            ...
            hass_icon = "mdi:leaf"
            hass_unit_of_meas = "%"
          elif "rssi" in key and not "-rssi" in key and not "lvl" in key:    # RSSI 
            hass_ent_cat = "diagnostic"
            dBm = intFallback(value,-999)
            if dBm >= -60: hass_icon = "mdi:wifi-strength-4"                 #    0 .. -60  --> 4  sehr gut
            elif dBm >= -80: hass_icon = "mdi:wifi-strength-3"               #  -61 .. -80  --> 3  gut
            elif dBm >= -90: hass_icon = "mdi:wifi-strength-2"               #  -81 .. -90  --> 2  durchschnittlich
            elif dBm >= -100: hass_icon = "mdi:wifi-strength-1"              #  -91 .. -100 --> 1 schwach
            elif dBm >= -110: hass_icon = "mdi:wifi-strength-alert-outline"  # -101 .. -110 --> alert sehr schlecht
            else: hass_icon = "mdi:wifi-strength-off-outline"                # -111 ..      --> n.a.
            hass_dev_cla = "signal_strength"
            hass_unit_of_meas = "dBm"
          elif "rssilvl" in key:                                             # RSSI-Level
            hass_ent_cat = "diagnostic"
            if value == "4": hass_icon = "mdi:wifi-strength-4"               #    0 .. -60  -->  4  sehr gut
            elif value == "3": hass_icon = "mdi:wifi-strength-3"             #  -61 .. -80  -->  3  gut
            elif value == "2": hass_icon = "mdi:wifi-strength-2"             #  -81 .. -90  -->  2  durchschnittlich
            elif value == "1": hass_icon = "mdi:wifi-strength-1"             #  -91 .. -100 -->  1  schwach
            elif value == "0": hass_icon = "mdi:wifi-strength-alert-outline" # -101 .. -110 -->  0  alert sehr schlecht
            else: hass_icon = "mdi:wifi-strength-off-outline"                # -111 ..      --> -1 n.a.
            hass_unit_of_meas = "Index"
          elif ("sig" in key or "-rssi" in key) and not "time" in key:
            hass_ent_cat = "diagnostic"
          ...
          elif "iot-" in key and "realtime_power" in key:        # IoT
            hass_icon = "mdi:meter-electric-outline"
            hass_dev_cla = "power"
            hass_unit_of_meas = "W"
          elif "iot-" in key and "ac_current" in key:            # IoT
            hass_icon = "mdi:meter-electric"
            hass_unit_of_meas = "mA"
          elif "iot-" in key and "flow_velocity" in key:         # IoT
            hass_icon = "mdi:air-filter"
            hass_dev_cla = "volume_flow_rate"
            hass_unit_of_meas = "L/min"
          elif "iot-" in key and "rfnet_state" in key:           # IoT
            hass_icon = "mdi:access-point" if value == "1" else "mdi:access-point-off"
            hass_ent_cat = "diagnostic"
          elif "iot-" in key and "-model" in key:                # IoT
            hass_icon = "mdi:pipe-valve" if value == "1" else "mdi:power-socket-de"
          elif "iot-" in key and "-always_on" in key:            # IoT
            hass_icon = "mdi:play" if value == "1" else "mdi:timer-play-outline"
          elif "iot-" in key and ("-ac_action" in key or "-water_action" in key):               # IoT
            hass_icon = "mdi:power"
          elif "iot-" in key and ("-devicename" in key or "-nickname" in key or "-plan_status" in key or "-id" in key or "-version" in key or "-capacity" in key or key[-4:] == "-ver"):  # IoT
            hass_icon = "mdi:information-outline"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          elif "iot-" in key and ("-ac_running" in key or "-ac_status" in key or "-water_running" in key or "-water_status" in key):                 # IoT
            hass_icon = "mdi:run" if value == "1" else "mdi:stop"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
            ...
            hass_state_cla = "total_increasing"
          elif "iot-" in key and ("-water_total" in key or "-happen_water" in key):             # IoT
            hass_icon = "mdi:gauge"
            hass_dev_cla = "water"
            hass_unit_of_meas = "L"
            hass_state_cla = "total_increasing"
          elif "iot-" in key and "-val_type" in key:             # IoT
            hass_icon = "mdi:size-m" if value == "1" else "mdi:size-s"
          elif "iot-" in key and key[-4:] == "-val":             # IoT
            hass_icon = "mdi:timer-stop-outline"
            hass_val_tpl = "{{ timedelta(seconds = " + value + ") }}"
            hass_state_cla = None
          elif ("uv" in key or "brightness" in key or "solarradiation" in key or "srsum" in key or "theosunsr" in key) and not "time" in key:
            ...
            elif "solarradiation" in key or key == "srsum" or key == "theosunsr":
            ...
            hass_icon = "mdi:sigma"
            hass_ent_cat = "diagnostic"
          elif ("ptrend" in key and not "text" in key) or "pchange" in key:
            ...
            if "pchange" in key:
              hass_dev_cla = "pressure"
              #hass_unit_of_meas = "hPa" if metric else "inHg"
              hass_unit_of_meas = "inHg" if "in" in key else "hPa"
          elif "ptrend" in key and "_text" in key:
            hass_icon = "mdi:information-slab-box-outline"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          # 28.03.24 - new
            ...
            hass_unit_of_meas = "B"
            hass_ent_cat = "diagnostic"
          elif key == "wprogtxt" or key == "wnowtxt":
            hass_icon = "mdi:information-slab-box-outline"
            hass_state_cla = None
          elif key == "stationtype" or key == "model" or key == "freq" or key == "PASSKEY" or key == "ws90_ver" or key == "ws85_ver" or key == "missed" or key == "battwarn" or key == "prgbuild" or key == "prgname" or key == "prgver" or key == "wnowcnd":
            hass_icon = "mdi:information-slab-box-outline"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          elif key == "alt" or key == "lat" or key == "lon":
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
            if key == "lat":
              hass_icon = "mdi:latitude"
            elif key == "lon":
              hass_icon = "mdi:longitude"
            elif key == "alt":
              hass_icon = "mdi:altimeter"
              hass_dev_cla = "distance"
              hass_unit_of_meas = "m" if metric else "ft"
            else:
              hass_icon = "mdi:information-slab-box-outline"
            ...
            hass_state_cla = None
          # LDS01 - v0.10
          elif "depth_ch" in key or "air_ch" in key:
            hass_icon = "mdi:arrow-expand-up" if "depth" in key else "mdi:arrow-expand-down"
            hass_unit_of_meas = "mm" if metric else "in"
            hass_dev_cla = "distance"
          elif "thi_ch" in key:
            hass_icon = "mdi:arrow-expand-down"
            hass_unit_of_meas = "mm" if metric else "in"
            hass_dev_cla = "distance"
          elif "ldsheat_ch" in key:
            hass_icon = "mdi:heating-coil"          #"mdi:mdi:counter"
            hass_unit_of_meas = ""         
          elif "vpd" in key:
            hass_icon = "mdi:water-thermometer"     # perhaps mdi:waves-arrow-up or mdi:water-thermometer-outline or mdi:water-percent better?
            hass_unit_of_meas = "kPa" if metric else "inHg"
            hass_dev_cla = "atmospheric_pressure"
          elif key == "wbgtcat":                    # WBGT category
            hass_icon = "mdi:numeric-"+value+"-box"
            hass_unit_of_meas = "Index"
          # create the payload
          elif "sunrise" in key or "sunset" in key:
            hass_icon = "mdi:weather-sunset-up" if key == "sunrise" else "mdi:weather-sunset-down"
            hass_state_cla = None
            hass_val_tpl = "{{ " + str(value) + " | " + "timestamp_custom(\"" + dtime_format + "\") }}"   # fallback: DT_FORMAT = "%d.%m.%Y %H:%M:%S"
          elif "sunheight" in key:
            hass_icon = "mdi:sun-angle-outline"
            hass_state_cla = None
            hass_unit_of_meas = "°"
            hass_state_cla = "measurement_angle"
          elif "sundir" in key:
            hass_icon = "mdi:sun-compass"
            hass_state_cla = None
            hass_unit_of_meas = "°"
            hass_state_cla = "measurement_angle"
          elif "theomaxsr" in key:
            hass_icon = "mdi:sun-wireless"
            hass_dev_cla = "irradiance"
            hass_unit_of_meas = "W/m²"
          elif "utc_offset" in key:
            hass_icon = "mdi:plus"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          elif "standard_tz" in key:
            hass_icon = "mdi:map-clock"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          elif "dst_active" in key:
            hass_icon = "mdi:map-clock-outline"
            hass_ent_cat = "diagnostic"
            hass_state_cla = None
          elif "-trigger" in key:
            doNothing                                          # will be patched later
          else:
            debugPrint("unknown HA key: "+key+"="+value)       # debugPrint
          ...
          if hass_dev_cla != None: payload.update({"device_class":hass_dev_cla})
          if hass_state_cla != None: payload.update({"state_class":hass_state_cla})
          if hass_val_tpl != None: payload.update({"val_tpl":hass_val_tpl})
          if hass_unit_of_meas != None: payload.update({"unit_of_measurement":hass_unit_of_meas})
          if hass_display_precision != None: payload.update({"suggested_display_precision":hass_display_precision})
          if useCategories and hass_ent_cat != None: payload.update({"ent_cat":hass_ent_cat})
          # topic: homeassistant/switch/my_device_name/config with payload { "name": "my_device_name", "command_topic": "homeassistant/switch/my_device_name/set", "state_topic": "homeassistant/switch/my_device_name/state"}
          if "ac_status" in key or "water_status" in key:
            payload.update({"pl_on":"1"})
            payload.update({"pl_off":"0"})
    
          if "-trigger" in key:
            # 2do: kein trigger im Standard-Array
            #tprint("---------- level: "+level+" prefix: "+prefix+" key: "+key+" value: "+value+" IOT_MQTT_LEVEL: "+IOT_MQTT_LEVEL)
            # try 16.08.24
            hass_trigger_state = key.replace("trigger","ac_status") if key[4:6] == "sp" else key.replace("trigger","water_status")
            #payload = { "name": key, "cmd_t": level + "/" + prefix+key, "stat_t": level + "/" + prefix+hass_trigger_state}
  
            payload = {
              "name": key,
              "unique_id": uniqid+"-"+key,
              "cmd_t": IOT_MQTT_LEVEL+key,
              "stat_t": level + "/" + prefix+hass_trigger_state,
              "dev":{
                "identifiers": [ uniqid ],
                "name": hass_dev_name,
                "mf": "Phantasoft",
            ...
            payload.update({"pl_on":"1"})
            payload.update({"pl_off":"0"})
            payload.update({"qos":mqtt_qos})
            payload.update({"retain":mqtt_retain})
  
            # Versuch - bewirkt aber nichts - wobei trigger ohnehin nicht angezeigt wird?!
            #payload.update({"ent_cat":"config"})
  
            d_out.append({'topic':'homeassistant/switch/'+uniqid+'/'+key+'/config','payload': json.dumps(payload)})
  
            # 28.08.2024
            # zusaetzlich Trigger in MQTT_LEVEL mit aktuellem Status versorgen
            # level: pingpong prefix:  key: iot-sp-0X28D6-trigger value: 0 IOT_MQTT_LEVEL: FOSHKplugin/Trigger/
            #tprint("*** key: "+key+" value: "+value)
            #print(iotDict)
            #print("key: "+key)
            #print("hass_trigger_state: "+hass_trigger_state)
            #print(iotDict[hass_trigger_state])
            if iotDict[hass_trigger_state] != value:
              d_out.append({'topic':IOT_MQTT_LEVEL+key,'payload': value})
              tprint("set "+IOT_MQTT_LEVEL+key+" to value " + str(value))
          else:
            # append the discovery topic
            d_out.append({'topic':'homeassistant/sensor/'+uniqid+'/'+key+'/config','payload': json.dumps(payload)})
  
          """
          # Test - status already written, so now create a separate trigger topic
          # append the discovery topic
          d_out.append({'topic':'homeassistant/sensor/'+uniqid+'/'+key+'/config','payload': json.dumps(payload)})
          if "ac_status" in key or "water_status" in key:
            # try 16.08.24
            hass_key = key.replace("ac_status","trigger") if key[4:6] == "sp" else key.replace("water_status","trigger")
            tprint("---------- level: "+level+" prefix: "+prefix+" key: "+key+" value: "+value+" IOT_MQTT_LEVEL: "+IOT_MQTT_LEVEL+" new key: "+hass_key)
            payload = { "name": key, "cmd_t": IOT_MQTT_LEVEL+hass_key, "stat_t": level + "/" + prefix+key}
            payload.update({"pl_on":"1"})
            payload.update({"pl_off":"0"})
            payload.update({"qos":"1"})
            payload.update({"retain":"true"})
...
def quoteString(s):                                            # turns a numeric value into a string; otherwise a double quotation mark is added at the front and back and any comma present is escaped
...
def processQueue(v, nr, d_in = {}, ignoreKeys = {}, remapKeys = {}, outstr = "", client = None, fwd_sid = ""):
  ...
  if v >= httpTries:                                           # save to file for later resend
    ...
    if "INFLUX" in qtype or ((qtype == "" or qtype == "TRUE") and "INFLUX" in fwd_type):
      ...
      if file_count > 0 and not processingQueue:
        processingQueue = True
        debugPrint("FWD-" + nr + ": process queued data (" + str(file_count) + " records) for " + dbname + "@" + srv + ":" + str(port))
        # Baustelle
        #tprint("FWD-" + nr + ": process queued data (" + str(file_count) + " records) for " + dbname + "@" + srv + ":" + str(port))
        for file_path in list_of_files:
          try:                                                 # qnd: prevent FileNotFoundError: [Errno 2] No such file or directory:
            with open(file_path) as f:
              outstr = f.read()                                # read file
              # Baustelle!
              #if "missed=wh90batt" in outstr:
              #  tprint("Baustelle! "+outstr)
              tprint("FWD-"+nr+": file_count: " + str(file_count) + " processingQueue: " + str(processingQueue) + " threads: " + str(threading.active_count()))
              outstr = outstr.replace("missed=wh90batt","missed=\"wh90batt\"")
            ...
            pass
        processingQueue = False                                # queue processing may be started
      else:
        if file_count > 0: tprint("FWD-"+nr+": file_count: " + str(file_count) + " processingQueue: " + str(processingQueue) + " threads: " + str(threading.active_count()))
  # done
  return qstr                                                  # processQueue
...
def sendToInfluxDB(client, iflstr, ver = 1, dbname = "", fwd_sid = ""):
  ...
  try:
    ...
    #DBwritten = True                                          # this was uncommented?! dst_active
    pass
  except Exception as err:
...
def forwardDictToInfluxDB(url,d_in,fwd_sid,fwd_pwd,status,script,nr,ignoreKeys,remapKeys,fwd_options,metric,ver=1):
  # initialize vars
  debugPrint("forwardDictToInfluxDB "+nr+" start")
  # v0.10 - gather options from FWD_OPTION
  o = stringToDict(fwd_options.replace(r"\,","[Komma]"),",",strip=True)
  allowdmin = mkBoolean(getfromDict(o,["allowdmin"],ignoreKeys,"False"))
  # allow "--" as value - ignoreKeys = blacklist keys; ignoreValues = empty values
  ignoreValues = ["-9999","None","null","",None] if allowdmin else ["-9999","None","null","",None,"--"]
  ...
  d.update(sunDict)                                            # additional sun data
  ...
  if status:                                                   # Baustelle
    statusstr = getStatusString(",",True, True)                # append status to line if status set - use false/true
    #tprint("status: "+str(status)+" -->"+"*"+statusstr+"*")
    iflstr += statusstr
...
#          buckets_api.create_bucket(bucket_name=dbname, org=fwd_sid)
#        except ApiException:                                   # ignore the error message when already existing
#          pass
      # Baustelle
      #debugPrint(dbname+": v"+str(ver)+" "+iflstr)
...
def WetterPrognose(diff,lang):
  arr=[
        ...
        ["Storm met hagel","Regen/storm","regenachtig","binnenkort regen","gelijkblijvend","lang mooi","mooi en onstabiel","Storm waarschuwing"],
...
def PM25toAQI(C):
  # according to https://aqicn.org/faq/2013-09-09/revised-pm25-aqi-breakpoints/de/
  if (type(C) != float):
    return(-9999)
  elif C < 12.1:                 # green #009966 - good
    I_high =  50
    I_low  =   0
    C_high =  12
    C_low  =   0
  elif C < 35.5:                 # yellow #ffde33 - moderate
    I_high = 100
    I_low  =  51
    C_high = 35.4
    C_low  = 12.1
  elif C < 55.5:                 # orange #ff9933 - Unhealthy for Sensitive Groups
    I_high = 150
    I_low  = 101
    C_high = 55.4
    C_low  = 35.5
  elif C < 150.5:                # red #cc0033 - Unhealthy
    I_high = 200
    I_low  = 151
    C_high = 150.4
    C_low  = 55.5
  elif C < 250.5:                # indigo purple #660099 - Very Unhealthy
    I_high = 300
    I_low  = 201
    C_high = 250.4
    C_low  = 150.5
  elif C < 350.5:                # braun #7e0023 - Hazardous
    I_high = 400
    I_low  = 301
    C_high = 350.4
    C_low  = 250.5
  else:                          # braun #7e0023 - Hazardous
    I_high = 500
    I_low  = 401
    C_high = 500.4
    C_low  = 350.5
...
def addSignalValues(adr):
  ...
  try:
    for i in range(1,5):                                       # GW1100 v2.4.5 (not official) has values on 4 pages - so range(1,3) adjusted to range(1,5)
      r = requests.get("http://"+adr+"/get_sensors_info?page="+str(i),timeout=ShorthttpTimeOut)        # timeout = httpTimeOut
      if r.status_code == 200:
        j = r.json()
        debugPrint("get_sensors_info: "+str(j))
        for item in j:
          if item["id"] != "FFFFFFFF" and item["id"] != "FFFFFFFE": 
            if item["name"][-4:-2] == "CH" and isNumeric(item["name"][-2]): ch = item["name"][-2:]
            elif item["name"][-3:-1] == "CH" and isNumeric(item["name"][-1]): ch = item["name"][-1]
            else: ch = ""
            debugPrint("addSignalValues: " + item["img"] + "sig" + ch + "=" + item["signal"])
            out += "&"+item["img"]+"sig"+ch+"="+item["signal"]
            if ADD_RSSI:
              try: 
                if item["rssi"]:
                  debugPrint("addSignalRssiValues: " + item["img"] + "rssi" + ch + "=" + item["rssi"])
                  out += "&"+item["img"]+"rssi"+ch+"="+item["rssi"]
              except:
                pass
            if ADD_RSSILVL:
              try: 
                if item["rssi"]:
                  dBm = intFallback(item["rssi"],-999)
                  if dBm >= -60: lvl = "4"                     #    0 .. -60  --> 4  sehr gut
                  elif dBm >= -80: lvl = "3"                   #  -61 .. -80  --> 3  gut
                  elif dBm >= -90: lvl = "2"                   #  -81 .. -90  --> 2  durchschnittlich
                  elif dBm >= -100: lvl = "1"                  #  -91 .. -100 --> 1 schwach
                  elif dBm >= -110: lvl = "0"                  # -101 .. -110 --> alert sehr schlecht
                  else: lvl = "-1"                             # -111 ..      --> n.a.
                  debugPrint("addSignalRssiLvlValues: " + item["img"] + "rssilvl" + ch + "=" + lvl)
                  out += "&"+item["img"]+"rssilvl"+ch+"="+lvl
              except:
                pass
      else: debugPrint("addSignalValues get_sensors_info: status_code: "+str(r.status_code))
  except Exception as e:
    debugPrint("addSignalValues get_sensors_info - exception: "+str(e))
    pass

  # Werner
  if ADD_MORE:
    try:
      r = requests.get("http://"+adr+"/get_device_info?",timeout=ShorthttpTimeOut)        # timeout = httpTimeOut
      if r.status_code == 200:
        j = r.json()
        #debugPrint("get_device_info: "+str(j))
        if j["radcompensation"]: 
          #debugPrint("getDevice_info: radcompensation="+j["radcompensation"])
          out += "&radcompensation="+j["radcompensation"]
        if j["newVersion"]: 
          #debugPrint("getDevice_info: newVersion="+j["newVersion"])
          out += "&newVersion="+j["newVersion"]
        if j["upgrade"]: 
          #debugPrint("getDevice_info: upgrade="+j["upgrade"])
          out += "&upgrade="+j["upgrade"]
      else: debugPrint("addSignalValues status_code: "+str(r.status_code))
    except Exception as e:
      debugPrint("addSignalValues get_device_info - exception: "+str(e))
      pass

    piezorain = False
    try:
      r = requests.get("http://"+adr+"/get_rain_totals?",timeout=ShorthttpTimeOut)        # timeout = httpTimeOut
      if r.status_code == 200:
        j = r.json()
        #debugPrint("get_rain_totals: "+str(j))
        if j['rainFallPriority']:
          #debugPrint("addRain_totals: rainFallPriority="+j["rainFallPriority"])
          out += "&rainFallPriority="+j["rainFallPriority"]
        if j['rainGain']:
          #debugPrint("addRain_totals: rainGain="+j['rainGain'])
          out += "&rainGain="+j['rainGain']
        if j["rstRainDay"]: 
          #debugPrint("addRain_totals: rstRainDay="+j["rstRainDay"])
          out += "&rstRainDay="+j["rstRainDay"]
        if j["rstRainWeek"]: 
          #debugPrint("addRain_totals: rstRainWeek="+j["rstRainWeek"])
          out += "&rstRainWeek="+j["rstRainWeek"]
        if j["rstRainYear"]: 
          #debugPrint("addRain_totals: rstRainYear="+j["rstRainYear"])
          out += "&rstRainYear="+j["rstRainYear"]
        if j["piezo"]: 
          #debugPrint("addRain_totals: piezo="+j["piezo"])
          out += "&piezo="+j["piezo"]
          if j["piezo"] == "1":
            piezorain = True
      else: debugPrint("addSignalValues get_rain_totals: status_code: "+str(r.status_code))
    except Exception as e:
      debugPrint("addSignalValues get_rain_totals - exception: "+str(e))
      pass

    if piezorain == True: 
      try:
        r = requests.get("http://"+adr+"/get_piezo_rain?",timeout=ShorthttpTimeOut)        # timeout = httpTimeOut
        if r.status_code == 200:
          j = r.json()
          #debugPrint("get_piezo_rain: "+str(j))
          if j["rain1_gain"]: 
            #debugPrint("addPiezo_rain: rain1_gain="+j["rain1_gain"])
            out += "&rain1_gain="+j["rain1_gain"]
          if j["rain2_gain"]: 
            #debugPrint("addPiezo_rain: rain2_gain="+j["rain2_gain"])
            out += "&rain2_gain="+j["rain2_gain"]
          if j["rain3_gain"]: 
            #debugPrint("addPiezo_rain: rain3_gain="+j["rain3_gain"])
            out += "&rain3_gain="+j["rain3_gain"]
          if j["rain4_gain"]: 
            #debugPrint("addPiezo_rain: rain4_gain="+j["rain4_gain"])
            out += "&rain4_gain="+j["rain4_gain"]
          if j["rain5_gain"]: 
            #debugPrint("addPiezo_rain: rain5_gain="+j["rain5_gain"])
            out += "&rain5_gain="+j["rain5_gain"]
        else: debugPrint("addSignalValues get_piezo_rain: status_code: "+str(r.status_code))
      except Exception as e:
        debugPrint("addSignalValues get_piezo_rain - exception: "+str(e))
        pass
...
def addDataToLine(line, what, newvalue, overwrite):
  ...
  newline = ""
  global min_max
  if not what in d.keys() or overwrite:
    #tprint("drin!")
    #if what == "addsignal": tprint(line)
    # gibt es noch nicht
    if what == "windchillf":
      try:
      ...
      try:
        co2 = float(getfromDict(d,["co2","co2in","co2_in"]))
        # CO2-level 1..6
    ...
    # v0.08
      ...
      try:
        interval = 0
        sunseconds = 0
        currtime = int(time.time())                            # 2do: check if UTC or local
        # bool field for sun is shining respecting hold time given in Config file
        # last_suntime is "" at start of the day (initMinMax)
        try:
          sunshine = 1 if int(min_max["last_suntime"]) + int(SUNSHINE_HOLD) >= currtime else 0
        except ValueError:
          sunshine = 0
        ...
        if sr != "null" and float(sr) >= sunths:
          ...
          if useSunCalc:
            if interval < 365:
              #tprint("sunhours:  " + str(currtime))
              sunseconds = calcSunduration(sr, interval, currtime)  # gibt Sekunden mit Sonne aus; keine Ahnung, was die 365 soll
            ...
            min_max["last_suntime"] = currtime if currtime > 0 else ""
            sunshine = 1
          #tprint("*** in sunhours: "+str(sunths)+" currtime: "+str(currtime)+" interval: "+str(interval)+" sunseconds: "+str(sunseconds)+" sunshine: "+str(sunshine)+" sr: "+str(sr)+" minmax: "+str(min_max["sunmins"]))
        # 2do: output sunhours everytime or just if there's a value > 0?
        # if min_max["sunmins"] > 0:
        sunhours = str(round(min_max["sunmins"]/60,2))
        #25.08.25 raus
        #tprint("*** in sunhours: sunhours: "+str(sunhours)+" sunths: "+str(sunths)+" currtime: "+str(currtime)+" interval: "+str(interval)+" sunseconds: "+str(sunseconds)+" sunshine: "+str(sunshine)+" sr: "+str(sr)+" minmax: "+str(min_max["sunmins"]))
        if sr != "null":
    ...
    elif what == "adddewpt":                                   # v0.10 additional dewpoints dewptinf, dewptNf, dewptf_co2 and dewptinc, dewptNc, dewptc_co2
      try:
        ...
        temp = floatFallback(getfromDict(d,["tf_co2"]))
        hum = floatFallback(getfromDict(d,["humi_co2"]))
        if isNumeric(temp) and isNumeric(hum):
          newvalue = getDewPointF(temp, hum)
      ...
      try:                                                     # v0.10 additional spread values spread, spreadin, spreadN, spread_co2 (same as metric)
        temp = floatFallback(getfromDict(d,["tempf"]))
        hum = floatFallback(getfromDict(d,["humidity"]))
        if isNumeric(temp) and isNumeric(hum):
      ...
      try:                                                     # v0.10 additional VPD values vpd, vpdin, vpdN, vpd_co2 (only if not present)
        temp = floatFallback(getfromDict(d,["tempf"]))
        hum = floatFallback(getfromDict(d,["humidity"]))
        if isNumeric(temp) and isNumeric(hum):
          vpd = calcVPDf(temp, hum, "inHg")
          #tprint("orig VPD: *"+getfromDict(d,["vpd"])+"*")
          if isNumeric(vpd) and getfromDict(d,["vpd"]) == "null": outstr += "&" + "vpd" + "=" + str(vpd)
          if HIDDEN_FEATURES and isNumeric(vpd) and getfromDict(d,["vpdout"]) == "null": outstr += "&" + "vpdout" + "=" + str(vpd)
          #tprint("after VPD: *"+getfromDict(d,["vpd"])+"*")
        temp = floatFallback(getfromDict(d,["tempinf"]))
        hum = floatFallback(getfromDict(d,["humidityin"]))
        if isNumeric(temp) and isNumeric(hum):
          vpd = calcVPDf(temp, hum, "inHg")
          if isNumeric(vpd) and getfromDict(d,["vpdin"]) == "null": outstr += "&" + "vpdin" + "=" + str(vpd)
        for i in range(1,9):
          temp = floatFallback(getfromDict(d,["temp"+str(i)+"f"]))
          hum = floatFallback(getfromDict(d,["humidity"+str(i)]))
          if isNumeric(temp) and isNumeric(hum):
            vpd = calcVPDf(temp, hum, "inHg")
            if isNumeric(vpd) and getfromDict(d,["vpd"+str(i)]) == "null": outstr += "&" + "vpd"+str(i) + "=" + str(vpd)
        temp = floatFallback(getfromDict(d,["tf_co2"]))
        hum = floatFallback(getfromDict(d,["humi_co2"]))
        if isNumeric(temp) and isNumeric(hum):
            vpd = calcVPDf(temp, hum, "inHg")
            if isNumeric(vpd) and getfromDict(d,["vpd_co2"]) == "null": outstr += "&" + "vpd_co2" + "=" + str(vpd)
      except (ValueError, KeyError): pass
    elif what == "addsignal":
      try:                                                     # v0.10 - get signal quality from supported console
        sig = addSignalValues(WS_IP)
        outstr += sig
      except (ValueError, KeyError): pass
    elif what == "wbgtcat":
      try:
        temp = floatFallback(getfromDict(d,["wbgt"]))          # imperial value °Fahrenheit
        if isNumeric(temp):
          if temp >= 90: newvalue = "5"                        # 5 = black  90 - ..   = 32.2 - ..
          elif temp >= 88: newvalue = "4"                      # 4 = red    88 - 89.9 = 31.1 - 32.1
          elif temp >= 85: newvalue = "3"                      # 3 = yellow 85 - 87.9 = 29.5 - 31.0
          elif temp >= 82: newvalue = "2"                      # 2 = green  82 - 84.9 = 27.8 - 29.4
          else: newvalue = "1"                                 # 1 = white  <= 81.9   = ..   - 27.7
          #debugPrint("addDataToLine/wbgtcat: "+str(temp) + "°F (" + str(ftoc(temp,1)) + "°C) cat: " + str(newvalue))
          outstr += "&" + what + "=" + str(newvalue)
      except (ValueError, KeyError): pass
    else:
      outstr += "&" + what + "=" + str(newvalue)
  ...
  #if what == "ptrend": tprint(newline)
  return newline                                               # addDataToLine
...
def forwardDictToLuftdaten(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys):
  ...
  while okstr[0:7] == "<ERROR>" and v < httpTries:
    try:
      headers={"X-Pin": "1", "X-Sensor": fwd_sid}
      if USER_AGENT != "": headers.update({"User-Agent": USER_AGENT})
      r = requests.post(url,
        json={
...
def addStatusToDict(d, makeBool=False):                        # add Status to dict as 0/1 or True/False (if makeBool
  ...
  if inBatteryWarning and BattWarnSensor != "": d.update({"battwarn" : BattWarnSensor})
...
def getStatusString(sep, makeBool=False, doQuote=False):       # output status string
  sw_what = ""
  bw_what = ""
  if inSensorWarning and SensorIsMissed != "":
    sw_what = sep+"missed="
    sw_what += "\""+SensorIsMissed+"\"" if doQuote else SensorIsMissed
  if inBatteryWarning and BattWarnSensor != "":
    bw_what = sep+"battwarn="
    bw_what += "\""+BattWarnSensor+"\"" if doQuote else BattWarnSensor
  if makeBool:
    s = "running=" + str(wsconnected) + sep + "wswarning=" + str(inWStimeoutWarning) + sep + "sensorwarning=" + str(inSensorWarning) + sw_what + sep + "batterywarning=" + str(inBatteryWarning) + bw_what + sep + "stormwarning=" + str(inStormWarning) + sep + "tswarning=" + str(inTSWarning) + sep + "updatewarning=" + str(updateWarning) + sep + "leakwarning=" + str(inLeakageWarning) + sep + "co2warning=" + str(inCO2Warning) + sep + "intvlwarning=" + str(inIntervalWarning) + sep + "time=" + str(loxTime(time.time()))
  else:
    s = "running=" + str(int(wsconnected)) + sep + "wswarning=" + str(int(inWStimeoutWarning)) + sep + "sensorwarning=" + str(int(inSensorWarning)) + sw_what + sep + "batterywarning=" + str(int(inBatteryWarning)) + bw_what + sep + "stormwarning=" + str(int(inStormWarning)) + sep + "tswarning=" + str(int(inTSWarning)) + sep + "updatewarning=" + str(int(updateWarning)) + sep + "leakwarning=" + str(int(inLeakageWarning)) + sep + "co2warning=" + str(int(inCO2Warning)) + sep + "intvlwarning=" + str(int(inIntervalWarning)) + sep + "time=" + str(loxTime(time.time()))
  return s
...
class RequestHandler(BaseHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    global myDebug, LEAKAGE_WARNING, CO2_WARNING, INTVL_WARNING, REBOOT_WARNING, FWD_WARNING, BATTERY_WARNING, BUT_PRINT, LOG_LEVEL, PO_ENABLE, POcustomWarning, inttime, UDP_TRIGGER, UDP_ONCHANGE
    ...
    # Lieferung im WU-Format
      ...
      if lastData > 0:
        ...
        if INTVL_WARNING:                                              # warn if measured interval is more than 10% above the agreed send interval
          if isinterval > INTVL_LIMIT and not inIntervalWarning:
            ...
            pushPrint("<RESTORED> real sending interval ("+str(isinterval)+") matches the interval set to the weather station ("+str(WS_INTERVAL)+") again")
      ...
      # v0.10 global remap in string
      instr = remappedString(instr,ADD_REMAP)
      ...
      if EVAL_VALUES:
        ...
        instr = addDataToLine(instr,"wbgtcat",None,False)      # v0.10: WBGT category
      ...
      # v0.10 some more new data points: dewptf, spread, signals
      if ADD_DEWPT: instr = addDataToLine(instr,"adddewpt",None,False)
      if ADD_SPREAD: instr = addDataToLine(instr,"addspread",None,False)
      if ADD_VPD: instr = addDataToLine(instr,"addvpd",None,False)       # v0.10: add VPD
      if ADD_SIGNAL: instr = addDataToLine(instr,"addsignal",None,False)
      ...
      # only send changed values via UDP to minimise traffic-  UDP_ONCHANGE
      d_use = d_m.copy() if USE_METRIC else d_e.copy()
      d_use.update({"wnowcnd": WeatherCondition(d_use)})
      d_udp = {}
      for key,value in d_use.items():
        if UDP_TRIGGER or (not UDP_ONCHANGE) or (UDP_ONCHANGE and (key not in last_d_all or value != last_d_all[key])):
          d_udp.update({key:value})
      # reset UDP send all trigger
      if UDP_TRIGGER: UDP_TRIGGER = False
      global last_d_e
      last_d_e = d_e
      global last_d_m
      last_d_m = d_m
      ...
      # v0.10: IoT data
      last_d_all.update(iotDict)

      # v0.10 - sun data
      # Baustelle!
      global sunDict
      if ADD_SUNDATA and HIDDEN_FEATURES: sunDict = solarInfo2(COORD_LAT, COORD_LON, COORD_ALT, linke_tl=SUN_TURBIDITY)
      last_d_all.update(sunDict)
      ...
      UDPstr = "SID=" + defSID + " " + dictToString(d_udp," ",True,UDP_IGNORE)
      #tprint(UDPstr)
        ...
        for i in range(len(fwd_arr)):                          # 0:url,1:interval,2:interval_num,3:last,4:ignore,5:type,6:fwd_sid,7:fwd_pwd,8:status,9:minmax,10:script,11:nr,12:mqttcycle,13:fwd_remap,14:fwd_option,15:fwd_cmt,16:lastok,17:errcount,18:code,19:warnint,20:queuetype,21:queuedir
          if time.time() >= fwd_arr[i][3]+fwd_arr[i][2]:
            ...
            if fwd_arr[i][5] == "WU":                          # String nach WU wandeln und per get versenden
              ...
              t.start()
            elif fwd_arr[i][5] == "WINDY":                      # convert to WU format and send to Windy.com v2 API
              t = threading.Thread(target=forwardStringToWindy, args=(fwd_arr[i][0],instr,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][8],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13]))
              t.start()
              ...
              t = threading.Thread(target=forwardDictToAwekas, args=(fwd_arr[i][0],d_m,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13],fwd_arr[i][14]))
              t.start()
              ...
              t = threading.Thread(target=forwardDictToInfluxDB, args=(fwd_arr[i][0],d_m,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][8],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13],fwd_arr[i][14],True,1))
              t.start()
            elif fwd_arr[i][5] == "INFLUXIMP":                 # send imperial dict to InfluxDB server
              t = threading.Thread(target=forwardDictToInfluxDB, args=(fwd_arr[i][0],d_e,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][8],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13],fwd_arr[i][14],False,1))
              t.start()
            elif fwd_arr[i][5] == "INFLUX2MET":                # send metric dict to InfluxDB2 server
              t = threading.Thread(target=forwardDictToInfluxDB, args=(fwd_arr[i][0],d_m,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][8],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13],fwd_arr[i][14],True,2))
              t.start()
            elif fwd_arr[i][5] == "INFLUX2IMP":                # send imperial dict to InfluxDB2 server
              t = threading.Thread(target=forwardDictToInfluxDB, args=(fwd_arr[i][0],d_e,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][8],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13],fwd_arr[i][14],False,2))
              t.start()
              ...
              t.start()
            elif fwd_arr[i][5] == "WINDGURU":                  # convert metric dict to WindGuru-API via GET
              t = threading.Thread(target=forwardDictToWindguru, args=(fwd_arr[i][0],d_m,fwd_arr[i][6],fwd_arr[i][7],fwd_arr[i][10],fwd_arr[i][11],fwd_arr[i][4],fwd_arr[i][13]))
              t.start()
      ...
      if CSVsave and time.time() >= last_csv_time + CSV_INTERVAL_num:
        ...
        csvline = lineToCSV(last_d_all,CSV_FIELDS)
        try:
          fcsv.write(csvline + "\r\n")
          fcsv.flush()
        except:
      ...
      # v0.07 now output empty fields also (needs CSV\CSV_FIELDS in config file)
        ...
        if "status" in request_path:
          ...
          bw_what = " battwarn=" + BattWarnSensor if inBatteryWarning and BattWarnSensor != "" else ""
          if htmlout != "": htmlout += sep
        ...
        if "iot" in request_path:
          if htmlout != "": htmlout += sep
          htmlout += dictToString(iotDict,sep,False,[],["null"],True,True,True)
        if "sun" in request_path:
          if htmlout != "": htmlout += sep
          htmlout += dictToString(sunDict,sep,False,[],["null"],True,True,True)
        ...
        if "iot" in request_path:
          if htmlout != "": htmlout += sep
          d_out.update(iotDict)
        if "sun" in request_path:
          if htmlout != "": htmlout += sep
          d_out.update(sunDict)
      ...
      # v0.08 realtime.txt
        ...
        if "iot" in request_path: d.update(iotDict)
        if "sun" in request_path: d.update(sunDict)
      ...
      # v0.07 Einzelabfrage von Werten
      ...
      elif "gettagfile" in request_path:
        file = getKeyFromURL("gettagfile",request_path)
        htmlout += fileTohtml(file, view = False)
      elif "getbannerimage" in request_path:
        file = getKeyFromURL("getbannerimage",request_path)
        htmlout += bannerTohtml(file)
      elif "observations" in request_path and "current" in request_path and "json" in request_path and "units=e" in request_path:
        htmlout += dictToWUServer(last_d_e,"&",False,getKeyFromURL("stationId",request_path),getKeyFromURL("apiKey",request_path))
      elif "observations" in request_path and "current" in request_path and "json" in request_path and "units=m" in request_path:
        htmlout += dictToWUServer(last_d_m,"&",True,getKeyFromURL("stationId",request_path),getKeyFromURL("apiKey",request_path))
        ...
        if "status" in request_path:
          if htmlout != "": htmlout += sep
          htmlout += getStatusString(sep, "bool" in request_path)
      elif "/WU" in request_path:                              # output data in WU format
        sep = getSeparator(request_path,"&")                   # separator from url
        wu_str = dictToWU(last_d_e,{})                         # convert to WU string
        my_d = dict(sorted(stringToDict(wu_str,"&").items())) if "sorted" in request_path else stringToDict(wu_str,"&")
        htmlout += dictToWU(my_d,{})
        if sep != "&": htmlout = htmlout.replace("&",sep)
        if "status" in request_path:
          if htmlout != "": htmlout += sep
          htmlout += getStatusString(sep, "bool" in request_path)
      elif "/IoT" in request_path and "/FOSHKplugin/" not in request_path:                             # output IoT data
        sep = getSeparator(request_path)                       # separator from url
        my_d = dict(sorted(iotDict.items())) if "sorted" in request_path else iotDict.copy()
        htmlout += dictToString(my_d,sep,True)
      elif "/Sun" in request_path and "/FOSHKplugin/" not in request_path:                             # output IoT data
        sep = getSeparator(request_path)                       # separator from url
        my_d = dict(sorted(sunDict.items())) if "sorted" in request_path else sunDict.copy()
        htmlout += dictToString(my_d,sep,True)
      ...
      elif "/FOSHKplugin/udponchange=enable" in request_path:
        UDP_ONCHANGE = True
        logPrint("<INFO> UDP_ONCHANGE enabled via http/get from " + request_addr)
        htmlout = "UDP_ONCHANGE enabled"
      elif "/FOSHKplugin/udponchange=disable" in request_path:
        UDP_ONCHANGE = False
        logPrint("<INFO> UDP_ONCHANGE disabled via http/get from " + request_addr)
        htmlout = "UDP_ONCHANGE disabled"
      elif "/FOSHKplugin/udponchange=trigger" in request_path:
        UDP_TRIGGER = True
        logPrint("<INFO> triggered one-time UDP full update via http/get from " + request_addr)
        htmlout = "triggered one-time UDP full update"
      ...
      # v0.10 - Prometheus support - metrics: all data; impmetrics: imperial data only; metmetrics: metric data only
      ...
      elif "/FOSHKplugin/IoT" in request_path:
        htmlout += operateIOT(request_path)
        ...
        if "/FOSHKplugin/fwdstat" in request_path:
          ...
          htmlout += "<h2>"+prgname+" "+prgbuild+" fwdstat"+RUN_DESC+"</h2>"
          ...
          myPort = str(LBH_PORT)
          htmlout += "<div style=\"overflow-x:auto;\">"
          htmlout += "<h2>"+prgname+" "+prgbuild+" help"+RUN_DESC+"</h2>"
          ...
          htmlout += tableRow("/FOSHKplugin/keyhelp","refresh, filter","shows all available (filtered) keys known to FOSHKplugin", myIP, myPort)
          htmlout += tableRow("/FOSHKplugin/tagfile","background, file, refresh","shows all tagfile outputs (you may specify the file or set the background)", myIP, myPort)
          ...
          htmlout += tableRow("/FOSHKplugin/getFullDict","separator, sorted, json, filter","get full (sorted, filtered) data dictionary as string", myIP, myPort)
          ...
          htmlout += tableRow("/FOSHKplugin/udponchange=disable","","send all data through UDP (UDP_ONCHANGE=False)", myIP, myPort)
          htmlout += tableRow("/FOSHKplugin/udponchange=enable","","only send changed data through UDP (UDP_ONCHANGE=True)", myIP, myPort)
          htmlout += tableRow("/FOSHKplugin/udponchange=trigger","","one-time sending full data set through UDP", myIP, myPort)
          ...
          htmlout += "<table><tr><th style=\"width:40%;\">url</th><th style=\"width:20%;\">options</th><th style=\"width:40%;\">comment</th></tr>"
          htmlout += tableRow("/","separator, units, minmax, status, bool, iot, sun","general overview as webpage", myIP, myPort)
          htmlout += tableRow("/CSVFILE","separator, units, minmax, status, bool, iot, sun","output with key=value as CSV", myIP, myPort)
          htmlout += tableRow("/DATA","refresh, separator, units, minmax, status, bool, iot, sun","get data as text", myIP, myPort)
          htmlout += tableRow("/IoT","refresh, separator","show IoT data", myIP, myPort)
          htmlout += tableRow("/JSON","refresh, units, minmax, status, bool, iot, sun","output as as JSON", myIP, myPort)
          htmlout += tableRow("/STRING","refresh, separator, units, minmax, status, bool, iot, sun","output as string", myIP, myPort)
          htmlout += tableRow("/Sun","refresh, separator","show additional sun data", myIP, myPort)
          htmlout += tableRow("/UDP","refresh, separator, units, minmax, status, bool, iot, sun","last UDP string is output via http", myIP, myPort)
          htmlout += tableRow("/getbannerimage=filename","refresh","download or show the generated banner named filename as an embeddable image", myIP, myPort)
          htmlout += tableRow("/gettagfile=filename","refresh","download or show the generated tagfile named filename", myIP, myPort)
          ...
          htmlout += tableRow("/WU","refresh, separator, status, bool","current data in Wunderground format", myIP, myPort)
          htmlout += tableRow("/observations/current/json/units=e","refresh, apiKey, stationId","WU-compatible data record with imperial values (&deg;F, mph, in, inHg)", myIP, myPort)
          htmlout += tableRow("/observations/current/json/units=m","refresh, apiKey, stationId","WU-compatible data record with metric values (&deg;C, kmh, mm, hPa)", myIP, myPort)
          ...
          htmlout += "<tr><td>&iot</td><td>include IoT data</td></tr>"
          htmlout += "<tr><td>&sun</td><td>include additional sun data</td></tr>"
          ...
          htmlout += "<tr><td>&locale=</td><td>define locale for for month and weekday names in selected language e.g. de_DE.UTF-8 (getvalue only)</td></tr>"
          htmlout += "<tr><td>&filter=</td><td>only returns the key/value combinations that match the search filter (keyhelp & getFullDict only)</td></tr></table>"
          ...
          htmlout += "<div style=\"overflow-x:auto;\">"
          htmlout += "<h2>"+prgname+" "+prgbuild+" keyhelp"+RUN_DESC+"</h2>"
          my_d = last_d_all

          # enable optional filter
          filtered = getURLvalue(request_path,"FILTER")
          if filtered != "":
            htmlout += "<h3>All filtered ("+filtered+") keys:</h3>\n"
            fil_d = {}
            for key, value in my_d.items():
              if filtered in key: fil_d.update({key:value})
            my_d = fil_d
          else:
          ...
          htmlout += "<div style=\"overflow-x:auto;\">\n"
          htmlout += "<h2>"+prgname+" "+prgbuild+" scanWS"+RUN_DESC+"</h2>\n"
          ...
          htmlout += "<div style=\"overflow-x:auto;\">\n"
          htmlout += "<h2>"+prgname+" "+prgbuild+" Banner"+RUN_DESC+"</h2>\n"
          ...
          if image_name == "":
            ...
            for i in range(len(list_of_files)):
              ...
              if firstLineIs(bannerconfig,"[Banner]"):
                forwardDictToBanner("",last_d_all,"","","","99B",{},{},"BANNER","bannerconfig="+bannerconfig)
                bannercfg = readConfigFile(bannerconfig)
                image_name = bannercfg.get('Banner','image_name',fallback='')
                if image_name != "":                             # only show if a banner config
                  c = "?" if "?" not in request_path else "&"
                  image_link = "<a href=\"http://"+myIP+":"+LBH_PORT+"/getbannerimage="+image_name+"\" target=\"_blank\">"
          ...
          htmlout += "\n</body>\n</html>"
        elif "/FOSHKplugin/tagfile" in request_path:            # v0.10 - tagfile watch ******
          htmlout += "<div style=\"overflow-x:auto;\">\n"
          htmlout += "<h2>"+prgname+" "+prgbuild+" Tagfile"+RUN_DESC+"</h2>\n"
          tagfileconfig = getURLvalue(request_path,"FILE")
          outfile = getURLvalue(request_path,"FILE")
          if tagfileconfig != "":
            try:
              forwardDictToTagfile("",last_d_all,"","","","99",{},{},"TAGFILE","config="+tagfileconfig)
              tagfilecfg = readConfigFile(tagfileconfig)
              outfile = tagfilecfg.get('Tagfile','outfile',fallback='')
            except: pass
          if outfile == "":
            # alle Config-Files nach [Tagfile] durchsuchen und die jeweiligen Bilder anzeigen
            myIP = LINK_ADR
            colors = ["white","lightgrey","lightblue"]
            clink = ""
            for color in colors: clink += "<a href=\"http://"+myIP+":"+LBH_PORT+"/FOSHKplugin/tagfile?background="+color+"\">"+color+"</a> "
            clink = clink[:-1]
            htmlout += "<h3>Show all configured tagfile files (max. "+str(maxViewsize)+" bytes): ("+clink+")</h3>\n"
            list_of_files = sorted( filter( os.path.isfile, glob.glob("*.conf") ) )
            for i in range(len(list_of_files)):
              tagfileconfig = list_of_files[i]
              if firstLineIs(tagfileconfig,"[Tagfile]"):
                forwardDictToTagfile("",last_d_all,"","","","99T",{},{},"TAGFILE","config="+tagfileconfig)
                tagfilecfg = readConfigFile(tagfileconfig)
                outfile = tagfilecfg.get('Tagfile','outfile',fallback='')
                if outfile != "":                             # only show if a tagfile config
                  c = "?" if "?" not in request_path else "&"
                  file_link = "<a href=\"http://"+myIP+":"+LBH_PORT+"/gettagfile="+outfile+"\" target=\"_blank\">"
                  config_link = "<a href=\"http://"+myIP+":"+LBH_PORT+request_path+c+"file="+tagfileconfig+"&refresh=10\" target=\"_blank\">"
                  htmlout += "file "+file_link+outfile+"</a> is created according to the config file "+config_link+tagfileconfig+"</a>:<br/><br/>\n"+fileTohtml(outfile,True)+"<br/><br/><hr>\n"
          else:                                                # show specified file
            htmlout += "<h3>Show tagfile: "+outfile+"</h3>\n"
            htmlout += fileTohtml(outfile, True)
          htmlout += "\n</body>\n</html>"
        else:
  ...
  def do_POST(self):
    ...
    if AUTH_PWD != "" and AUTH_PWD not in instr:
      ...
      global last_RAWstr, inttime, UDP_TRIGGER
      last_RAWstr = instr
      ...
      if ADD_DEWPT: instr = addDataToLine(instr,"adddewpt",None,False)
      if ADD_SPREAD: instr = addDataToLine(instr,"addspread",None,False)
      if ADD_VPD: instr = addDataToLine(instr,"addvpd",None,False)
      if ADD_SIGNAL: instr = addDataToLine(instr,"addsignal",None,False)
      if FIX_LIGHTNING and last_lightning_time != 0:
        # set empty keys to last known values
        instr = fixEmptyValue(instr,"lightning_time",str(last_lightning_time))
...
def sendToWS(ws_ipaddr, ws_port, cmd):           # oeffnet jeweils einen neuen Socket und verschickt cmd; Rueckmeldung = Rueckmeldung der WS
  ...
  v = 0
  data = ""
  debugPrint("sendToWS: " + str(ws_ipaddr)+":"+str(ws_port)+" command: " + str(cmd))
  while data == "" and v <= tries:
    ...
    try:
      ...
      debugPrint("Error in sendToWS for command " + str(cmd) + ": " + str(e))               # Baustelle: to check: is debugPrint already available?
      pass
    v +=1
  return data
...
def checkWS_report():
  ...
  if last_ws_time > 0:
    ...
    if UDP_STATRESEND > 0 and time.time() >= UDP_STATRESEND_time + UDP_STATRESEND:
      sw_what = " missed=" + "\"" + SensorIsMissed + "\"" if inSensorWarning and SensorIsMissed != "" else ""
      bw_what = " battwarn=" + "\"" + BattWarnSensor + "\"" if inBatteryWarning and BattWarnSensor != "" else ""
      statestr = "SID=" + defSID + " running=" + str(int(wsconnected)) + " wswarning=" + str(int(inWStimeoutWarning)) +  " sensorwarning=" + str(int(inSensorWarning)) + sw_what + " batterywarning=" + str(int(inBatteryWarning)) + bw_what + " stormwarning=" + str(int(inStormWarning)) + " tswarning=" + str(int(inTSWarning)) + " updatewarning=" + str(int(updateWarning)) + " leakwarning=" + str(int(inLeakageWarning)) + " co2warning=" + str(int(inCO2Warning)) + " intvlwarning=" + str(int(inIntervalWarning))
      #print(statestr)
      sendUDP(statestr)
...
def generateMinMax(d):                                         # fill the min/max array with current values and send via UDP
  ...
  if not thisDay(min_max["minmax_init"]):
    ...
    # v0.10 reset last data if a day change was recognized - ensure that the daily rainfall value is output correctly
    #logPrint("<DEBUG> d_m before:   " + str(last_d_m))
    last_d_m = d.copy()
    #logPrint("<DEBUG> d_m after:    " + str(last_d_m))
  ...
  for i in range(1,17):
    UDPstr += calcMinMax(getfromDict(d,["soilmoisture"+str(i),"soil_ec_hum"+str(i)]),"soilmoisture"+str(i),is_time)
  for i in range(1,9):
...
def POcustomLine(s):                                          # 08.02.
  ...
  if s[:2] == "@[":                                            # function detected - escape commas in function
   ...
   """
  s = s.replace(r"\,","%%").replace("$","#")                    # escape , and convert #
  cond = text = ""
  ena = True
  hold = 3600
  i = s.find(",")                                              # condition
    ...
    if what != "":
      ...
      try: locale.setlocale(locale.LC_TIME, locale_format)       # set locale for correct date output
        ...
        if not errMsgDone and sndlog:
          ...
          errMsgDone = True
        pass
      #what = bannerconfig.get('Banner',source+'_'+i_str,fallback='').replace(r"\,","[Komma]").replace("$datetime",fmt(time.strftime(dtime_format,time.localtime()),pre,"",dtime_format,locale_format)) if bannerconfig.has_option('Banner',source+'_'+i_str) else ""
      what = bannerconfig.get('Banner',source+'_'+i_str,fallback='').replace(r"\,","[Komma]").replace("[Hash]","#").replace("$datetime",fmt(time.strftime(dtime_format,time.localtime()),pre,"",dtime_format,locale_format)).replace("$","#") if bannerconfig.has_option('Banner',source+'_'+i_str) else ""
      #tprint(what)
...
def forwardDictToBanner(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys,fwd_type,fwd_options):
  ...
  dtime_format = bannerconfig.get('Banner','dtime_format',fallback='%d.%m.%Y %H:%M:%S').replace("\"","").replace(r"\,","[Komma]").replace(",","[Komma]")
...
def fileTohtml(filename, view = False):
  try:
    with open(filename, 'r') as file: data = file.read()
    if view:
      data = data[:maxViewsize] # Baustelle
      data = data.replace("\n","<br>")
      s = "<div style=\"display: -webkit-box; -webkit-line-clamp: 10; -webkit-box-orient: vertical; overflow: scroll; text-overflow: ellipsis; border: 1px solid #000; padding: 10px; width: 95%; background: lightgrey;\">\n"
      s += data
      s += "</div>"
    else: s = data
  except FileNotFoundError: s = "File "+filename+" not found!<br>" if view else "File "+filename+" not found!"
  return s
...
def addMoreToDict(d_in, lang):                                 # add some more fields to the given array ******
  ...
  try:
    fields_to_add = ["prgname", "prgver", "prgbuild", "winddir_text", "pchange1", "pchange3", "lightningmi", "aqtime", "starttime", "wnowcnd", "theosunsr", "theouv", "ptrend_text", "coords"]
    for field in fields_to_add:
      ...
      elif field == "wnowcnd": d_out.update({field: WeatherCondition(d_in)})
      elif field == "theosunsr": d_out.update({field: str(round(theoreticalSun(int(time.time())),2))})
      elif field == "theouv":
        tsr = round(theoreticalSun(int(time.time())),2)
        if tsr > 0:
          theouv = int(round(tsr/100*1,0))
          d_out.update({field: str(theouv)})
          #tprint("theoSun: " + str(tsr) + " UVnow: " + getfromDict(d_in,["uv"],ignoreKeys) + " UVthe: " + str(theouv))
      elif field == "ptrend_text":
        value = ptrendText(d_in, "ptrend1", lang)
        if value != "null": d_out.update({"ptrend1_text": value})
        value = ptrendText(d_in, "ptrend3", lang)
        if value != "null": d_out.update({"ptrend3_text": value})
      elif field == "coords":
        if COORD_LAT != "": d_out.update({"lat": COORD_LAT})
        if COORD_LON != "": d_out.update({"lon": COORD_LON})
        if COORD_ALT != "": d_out.update({"alt": COORD_ALT})
...
def forwardDictToTagfile(url,d_in,fwd_sid,fwd_pwd,script,nr,ignoreKeys,remapKeys,fwd_type,fwd_options):
  ...
  if configfile != "" and os.path.exists(configfile):
    ...
    dtime_format = tagconfig.get('Tagfile','dtime_format',fallback=dtime_format).replace("\"","").replace(r"\,",",")
...
def gatherIOTdata():
  adr = WS_IP
  debugPrint("gatherIOTdata " + adr + " start")
  global iotDict
  d = {}
  #if True:
  try:
    r = requests.get("http://"+adr+"/get_iot_device_list",timeout=httpTimeOut)
    if r.status_code == 200:
      j = r.json()
      for item in j["command"]:
        id = str(item["id"])
        id_hex = str(hex(item["id"])).upper()
        model = str(item["model"])
        if model == "1": dev = "iot-" + "wt" + "-" + id_hex
        elif model == "2": dev = "iot-" + "sp" + "-" + id_hex
        else: dev = "iot-" + "unk" + "-" + id_hex
        for key,value in item.items():
          if key != "cmd": d.update({dev+"-"+key : str(value)})
        headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Connection': 'Close', 'User-Agent': None}
        r = requests.post("http://"+adr+"/parse_quick_cmd_iot", data = '{"command":[{"cmd":"read_device","id":'+id+',"model":'+model+'}]}', headers=headers, timeout=httpTimeOut)
        #tprint("POST: "+"http://"+adr+"/parse_quick_cmd_iot"+" --> "+'{"command":[{"cmd":"read_device","id":'+id+',"model":'+model+'}]}')
        j = r.json()
        for item in j["command"]:
          for key,value in item.items():
            d.update({dev+"-"+key : str(value)})
        # add loxtime
        if LOX_TIME:
          try:
            d.update({dev+"-"+"loxtimeutc":str(loxTime(d[dev+"-"+"timeutc"]))})
            d.update({dev+"-"+"loxpublish_time":str(loxTime(d[dev+"-"+"publish_time"]))})
          except KeyError: pass
        # add device trigger
        if IOT_MQTT:
          state = str(item["ac_status"]) if model == "2" else str(item["water_status"])
          #tprint("IOT_MQTT: dev: " + dev + " model: " + model + " state: " + state)
          d.update({dev+"-"+"trigger" : state})
  except Exception as e:
    debugPrint("gatherIOTdata " + adr + " except: "+str(e))
    #print("except: " + str(e))
    pass
  ...
  # only send changed values via UDP to minimise traffic
  for key,value in d.items():
    if UDP_TRIGGER or (not UDP_ONCHANGE) or (UDP_ONCHANGE and (key not in last_iotDict or value != last_iotDict[key])):
      iot_udp.update({key:value})

  # jetzt UDPstr versenden
  if len(iot_udp) > 0:
    UDPstr = "SID=" + defSID + " " + dictToString(iot_udp," ",True,UDP_IGNORE)
    #25.08.25 raus
    #tprint("gatherIOTdata: " + UDPstr)
    sendUDP(UDPstr)
  last_iotDict = iotDict.copy()

  # instant MQTT
  
  debugPrint("gatherIOTdata " + adr + " stop")
  return                                                       # gatherIOTdata

def operateIOT(s):
  s = s.replace("/FOSHKplugin/","")                            # it's "IoT." for UDP but "/FOSHKplugin/IoT." in http
  try:
    duration = "0"
    cmds = s.split("&")                                        # commands are splitted with "&"
    for cmd in cmds:
      key, value = cmd.split("=")
      if key[:4] == "IoT.":
        what = key[4:]
        iot, typ, nr = value.split("-")                        # iot: IoT, typ: sp/wt, nr: 0x28c7
        if typ == "sp": model = "2"                            # sp = smart plug = AC1100 = model:2
        elif typ == "wt": model = "1"                          # wt = water timer = WFC01 = model:1
      elif key == "duration":
        duration = value
    always = "1" if duration == "0" else "0"
    val_type = "1" if duration == "0" else "0"
    nr = str(int(nr,16)) if "0x" in nr else nr                 # id must be decimal!
    nr_hex = (hex(int(nr)))
    if what == "start":
      post_str = '{"command":[{"on_type":0,"off_type":0,"always_on":' + always + ',"on_time":0,"off_time":0,"val_type":' + val_type + ',"val":' + duration + ',"cmd":"quick_run","id":' + nr + ',"model":' + model + '}]}'
      out_str = "start command for IoT device " + nr_hex + " has been sent"
    elif what == "stop":
      post_str = '{"command":[{"cmd":"quick_stop","id":' + nr + ',"model":' + model + '}]}'
      out_str = "stop command for IoT device " + nr_hex + " has been sent"
    else:
      out_str = "unknown command " + what
    # now send the request via http/post
    if what == "start" or what == "stop":
      headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Connection': 'Close', 'User-Agent': None}
      r = requests.post("http://"+WS_IP+"/parse_quick_cmd_iot", data = post_str, headers=headers, timeout=httpTimeOut)
  except Exception as e:
    out_str = str(e)
    logPrint("<ERROR> operateIOT reports: " + out_str)
  return out_str                                               # operateIOT

def justTopicName(s):                                          # return string behind last /
  try: t = s[len(s)-s[::-1].index("/"):]
  except: t = s
  return str(t)                                                # justTopicName

def on_mqtt_message(client, userdata, message):
  msg = str(message.payload.decode("utf-8"))
  on_msg = ["ON","on","1"]
  off_msg = ["OFF","off","0"]
  dev = justTopicName(message.topic).replace("-trigger","").replace("0X","0x")
  cmd = "IoT.start=" if msg in on_msg else "IoT.stop="
  tprint("MQTT message received: " +cmd+dev+" msg: *"+msg+"*")
  operateIOT(cmd+dev)
  # Problem:
  # die Werte sollten JETZT erneut per MQTT veroeffentlicht werden - sonst dauert eine Statusaenderung ggf. zu lange
  gatherIOTdata()

def on_mqtt_connect(client, userdata, flags, rc, prop = None):
  logPrint("<OK> Connected to MQTT Broker: " + IOT_MQTT_BROKER + " subscribed to: " + IOT_MQTT_LEVEL + "# with QOS " + str(IOT_MQTT_QOS))
  TOPIC = IOT_MQTT_LEVEL+"#",IOT_MQTT_QOS
...
if option == '-SCANWS':
  ...
  print()
  sys.exit(0)
elif "VERSION" in option or option == "-V" or option == "--V":
  print()
  ...
  print("-version                                  show FOSHKplugin version string")
  print()
  sys.exit(0)
...
# for Docker
if "FOSHKPLUGINCONFIGFILE" in os.environ:
  CONFIG_FILE = os.environ.get("FOSHKPLUGINCONFIGFILE")
  CONFIG_DIR = os.path.dirname(os.path.realpath(CONFIG_FILE))
  #print("dir: "+CONFIG_DIR+" file: "+CONFIG_FILE)
else:
...
# v0.10: fallback to "" instead of "LB_IP"
LB_IP = config.get('Config','LB_IP',fallback="")
...
UDP_ONCHANGE = mkBoolean(config.get('Config','UDP_ONCHANGE',fallback="True"))
...
ADD_VPD = mkBoolean(config.get('Export','ADD_VPD',fallback="False"))
ADD_SIGNAL = mkBoolean(config.get('Export','ADD_SIGNAL',fallback="False"))
ADD_RSSI = mkBoolean(config.get('Export','ADD_RSSI',fallback="False"))
ADD_RSSILVL = mkBoolean(config.get('Export','ADD_RSSILVL',fallback="False"))
ADD_MORE = mkBoolean(config.get('Export','ADD_MORE',fallback="False"))
ADD_SUNDATA = mkBoolean(config.get('Export','ADD_SUNDATA',fallback="True"))
# v0.10 global remap for all forwards
ADD_REMAP_STR = config.get('Export',"ADD_REMAP",fallback="").replace("\"","")
try:
  ADD_REMAP = dict(x.split("=") for x in ADD_REMAP_STR.split(",")) if ADD_REMAP_STR != "" else {}
except ValueError:
  ADD_REMAP = {}
  pass
...
BattWarnSensor = config.get('Status','BattWarnSensor',fallback="")
...
CO2_WARNKEY = config.get('Warning','CO2_WARNKEY',fallback='co2,co2in,co2_in,co2_in_aqin').replace("\"","").split(",")
...
SUN_MIN = config.get('Sunduration','SUN_MIN',fallback=12)       ##
SUN_COEF = config.get('Sunduration','SUN_COEF',fallback=0.92)  ## v0.10 lt. Werner besserer Wert
SUN_TURBIDITY = config.get('Sunduration','SUN_TURBIDITY',fallback=None)
SUN_TURBIDITY = floatFallback(SUN_TURBIDITY,None)
...
# v0.10 additional info output in pushPrint and in help, banner, fwdstat, keyhelp, scanWS in headline
RUN_DESC = config.get('Config','RUN_DESC',fallback='')
if RUN_DESC != "": RUN_DESC = " (" + RUN_DESC + ") "
# v0.10 snow height - defaults to depth_ch1
SNOW_DEPTH = config.get('Weatherstation','SNOW_DEPTH',fallback='notAvail')
USER_AGENT = config.get('Config','USER_AGENT',fallback="").replace("\"","")
USER_AGENT = "@@@SKIP_HEADER@@@" if USER_AGENT == "None" else USER_AGENT
...
# v0.10 IoT
IOT_ENABLE = mkBoolean(config.get('IoT','IOT_ENABLE',fallback="False"))
IOT_INTERVAL = config.get('IoT','IOT_INTERVAL',fallback="10")
IOT_INTERVAL = intFallback(IOT_INTERVAL,10)
IOT_MQTT = mkBoolean(config.get('IoT','IOT_MQTT',fallback="False"))
IOT_MQTT_BROKER = config.get('IoT','MQTT_BROKER',fallback="")
IOT_MQTT_PORT = intFallback(config.get('IoT','MQTT_PORT',fallback="1883"),1883)
IOT_MQTT_USER = config.get('IoT','MQTT_USER',fallback="")
IOT_MQTT_PASS = config.get('IoT','MQTT_PASS',fallback="")
IOT_MQTT_TOPIC = config.get('IoT','MQTT_TOPIC',fallback="")
IOT_MQTT_LEVEL = config.get('IoT','MQTT_LEVEL',fallback="FOSHKplugin/Trigger")
if IOT_MQTT_LEVEL[-1] != "/": IOT_MQTT_LEVEL = IOT_MQTT_LEVEL + "/"
IOT_MQTT_QOS = intFallback(config.get('IoT','MQTT_QOS',fallback="0"),0)
...
for i in range(0, maxfwd+1):                                   # +1 because stop not included
  ...
  if config.has_section(section):
    ...
    if fwd_enable and (fwd_url != "" or fwd_exec != "" or fwd_option != ""):       # v0.07: enable/disable manually
      ...
      if URL_REPAIR and fwd_type in ["WU", "WINDY", "RAW", "EW", "RAWEW", "EWRAW", "LD", "RAWCSV", "CSVRAW", "CSV", "AMB", "RAWAMB", "AMBRAW", "MT", "WC", "AWEKAS", "WETTERCOM", "WEATHER365", "WETTERSEKTOR"]:
...
if option == '-SETWSCONFIG':
  ...
  print("Patching the W4L configuration is no longer necessary - simply activate the FOSHKplugin grabber within the W4L configuration.")
  sys.exit(0)
...
allPrint("<OK> "+prgname+" "+prgbuild+" started"+RUN_DESC)
  ...
  if os.path.exists(fname) and int(time.time()) - lastStopTime < 600:                    # file exists; last stop < 10 minutes --> stundenwerte are current
    with open(fname, 'rb') as input:
      try:
        ...
        logPrint("<OK> loaded stundenwerte data from " + fname + " (" + str(len(stundenwerte)) + ")")
      except:
        stundenwerte = deque(maxlen=(int(3*3600/int(WS_INTERVAL))))
        logPrint("<WARNING> unable to load stundenwerte data from " + fname)
        pass
  else:
    stundenwerte = deque(maxlen=(int(3*3600/int(WS_INTERVAL))))
...
# v0.10 load data for clientraw.txt
fname = CONFIG_DIR+"/"+prgname+"-"+LBH_PORT+"-lasthour.pkl"
if os.path.exists(fname) and int(time.time()) - lastStopTime < 600:                      # file exists; last stop < 10 minutes --> stundenwerte are current
  with open(fname, 'rb') as input:
    try:
      lasthour = pickle.load(input)
      logPrint("<OK> loaded lasthour data from " + fname + " (" + str(len(lasthour)) + ")")
    except:
      lasthour = deque(maxlen=10)
      for i in range(len(lasthour)+1): lasthour.append(["--","--","--","--"])
      logPrint("<WARNING> unable to load lasthour data from " + fname)
      pass
else:
  lasthour = deque(maxlen=10)
  for i in range(lasthour.maxlen+1): lasthour.append(["--","--","--","--"])
...
if ADD_VPD:
  logPrint("<OK> additional VPD calculation (indoor sensor, WH31, WH45) activated - to disable set Export\\ADD_VPD = False in config")
else:
  logPrint("<OK> additional VPD calculation (indoor sensor, WH31, WH45) is deactivated - to enable set Export\\ADD_VPD = True in config")
...
if USER_AGENT != "":
  if USER_AGENT == "None":
    logPrint("<OK> don\'t use a User-Agent in http requests")
  else: 
    logPrint("<OK> using \""+USER_AGENT+"\" as User-Agent in http requests")
...
# v0.10 IoT
if IOT_ENABLE:
  logPrint("<OK> IoT processing enabled, polling " + WS_IP + " every " + str(IOT_INTERVAL) + " seconds - to disable set IoT\\IOT_ENABLE = False in config")
  t = threading.Thread(target=gatherIOTdata)
  t.start()
  gatherIOT = InfiniteTimer(IOT_INTERVAL, gatherIOTdata)
  gatherIOT.start()
else:
  logPrint("<OK> IoT processing disabled - to enable set IoT\\IOT_ENABLE = True in config")
...
if wsconnected:
  ...
  oc_str = " - only changes" if UDP_ONCHANGE else " - send all"
  logPrint("<OK> remote UDP: " + LOX_IP + ":" + LOX_PORT + oc_str + " (fragmented max len " + str(UDP_MAXLEN) + ")") if UDP_ENABLE else logPrint("<OK> remote UDP-sending disabled")
  ...
  if IOT_MQTT:
    #time.sleep(1)
    # better compatibility with paho >= v2.0
    try:
      mqtt_client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2)
      #mqtt_client = mqtt.Client(mqttClient.CallbackAPIVersion.VERSION1)
    except:
      mqtt_client = mqtt.Client()
    mqtt_client.username_pw_set(IOT_MQTT_USER, IOT_MQTT_PASS)
    mqtt_client.on_connect = on_mqtt_connect
    time.sleep(1)
    mqtt_client.on_message = on_mqtt_message
    mqtt_client.connect(IOT_MQTT_BROKER, IOT_MQTT_PORT)
    mqtt_client.loop_start()
    logPrint("<OK> IoT MQTT subscriber for broker " + IOT_MQTT_BROKER + ":" + str(IOT_MQTT_PORT) + " on topic " + IOT_MQTT_LEVEL + "#" + " started - to disable set IoT\\IOT_MQTT = False in config")
      ...
      if anzahl > 0:
        ...
        if data[0] == "SID=FOSHKplugin":                       # ist eine zu behandelnde Nachricht
          if data[1] == "System.reboot":                       # reboot-Request
            ...
            sw_what = " missed=" + "\"" + SensorIsMissed + "\"" if inSensorWarning and SensorIsMissed != "" else ""
            bw_what = " battwarn=" + "\"" + BattWarnSensor + "\"" if inBatteryWarning and BattWarnSensor != "" else ""
            sendUDP("SID=" + defSID + " running=" + str(int(wsconnected)) + " wswarning=" + str(int(inWStimeoutWarning)) +  " sensorwarning=" + str(int(inSensorWarning)) + sw_what + " batterywarning=" + str(int(inBatteryWarning)) + bw_what + " stormwarning=" + str(int(inStormWarning)) + " tswarning=" + str(int(inTSWarning)) + " updatewarning=" + str(int(updateWarning)) + " leakwarning=" + str(int(inLeakageWarning)) + " co2warning=" + str(int(inCO2Warning))  + " intvlwarning=" + str(int(inIntervalWarning)) + " time=" + str(loxTime(time.time())))
          ...
          # v0.10 enable/disable battery warning - BATTERY_WARNING
            ...
            BATTERY_WARNING = False
          elif data[1] == "Plugin.udponchange=enable":         # activate UDP_ONCHANGE
            logPrint("<INFO> UDP_ONCHANGE enabled via http/get from " + r_addr)
            UDP_ONCHANGE = True
          elif data[1] == "Plugin.udponchange=disable":        # disable UDP_ONCHANGE
            logPrint("<INFO> UDP_ONCHANGE disabled via http/get from " + r_addr)
            UDP_ONCHANGE = False
          elif data[1] == "Plugin.udponchange=trigger":        # disable UDP_ONCHANGE
            logPrint("<INFO> one-time sending of full UDP data triggered via http/get from " + r_addr)
            UDP_TRIGGER = True
          # v0.10 IoT device start/stop
          elif data[1][:4] == "IoT.":                          # handle IoT requests
            tprint("UDP with IoT. received")
            cmd = data[1].split("&")
            ret = operateIOT(data[1])
            ret = "<OK> " + ret if ret[1:] != "<" else ret
            logPrint(ret + " via UDP from " + r_addr)
    except:
      if sndlog :
        doNothing()
...
if IOT_ENABLE: gatherIOT.cancel()
...
# v0.10 clientraw
fname = CONFIG_DIR+"/"+prgname+"-"+LBH_PORT+"-lasthour.pkl"
debugPrint("write lasthour data to " + fname)
try:
  with open(fname, "wb") as output:
    try:
      pickle.dump(lasthour, output, pickle.HIGHEST_PROTOCOL)
      logPrint("<OK> wrote lasthour data to " + fname + " (" + str(len(lasthour)) + ")")
    except:
      logPrint("<ERROR> unable to write lasthour data to " + fname)
except OSError as e:
  logPrint("<ERROR> unable to write lasthour data to " + fname + ": " + str(e))
  pass

sendUDP("SID=" + defSID + " running=0")
allPrint("<OK> "+prgname+" "+prgbuild+" stopped"+RUN_DESC)
